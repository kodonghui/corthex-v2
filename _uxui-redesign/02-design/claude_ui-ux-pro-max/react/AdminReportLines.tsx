...
{/* GET /api/admin/report-lines — returns supervisor/subordinate pairs with delegation_scope, interval */}
  {/* Sidebar */}
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/admin/report-lines" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg>보고체계</a>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">보고 체계</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">38개 위임 라인</span>
      <div className="ml-auto flex items-center gap-2">
        <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-md">+ 보고선 추가</button>
      </div>
    </div>
    <div className="flex-1 p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">보고 라인 수</div>
          <div className="text-2xl font-semibold font-mono">38</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">위임 깊이 (최대)</div>
          <div className="text-2xl font-semibold font-mono">4 단계</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">단절 에이전트</div>
          <div className="text-2xl font-semibold font-mono text-yellow-400">2</div>
        </div>
      </div>

      <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-zinc-800">
          <span className="text-sm font-medium">보고 체계 목록</span>
          <span className="text-xs text-zinc-600 ml-2">— 상위 → 하위 위임 구조</span>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-800">
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">상위 (supervisor)</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">하위 (subordinate)</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">위임 권한</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">보고 주기</th>
            ...
            <tr className="hover:bg-zinc-800/30">
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 bg-zinc-400 rounded-full flex items-center justify-center text-xs text-black font-medium">김</div>
                  <span className="text-zinc-300 text-xs">김민준 (팀장)</span>
                </div>
              </td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center text-xs text-white">A</div>
                  <span className="font-mono text-blue-400 text-xs">AGT-001</span>
                  <span className="text-zinc-500 text-xs">퀀트마스터</span>
                </div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-blue-500/10 text-blue-400">주문 · 분석</span></td>
              <td className="px-4 py-3 text-xs text-zinc-400">1시간</td>
              <td className="px-4 py-3 text-xs text-zinc-400">AI 전략실</td>
            ...
            <tr className="hover:bg-zinc-800/30">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-zinc-500 text-xs">시그널헌터</span>
                </div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-purple-500/10 text-purple-400">신호 생성</span></td>
              <td className="px-4 py-3 text-xs text-zinc-400">즉시</td>
              <td className="px-4 py-3 text-xs text-zinc-400">AI 전략실</td>
              <td className="px-4 py-3"><button className="text-xs text-blue-400 hover:text-blue-300">편집</button></td>
            </tr>
            <tr className="hover:bg-zinc-800/30">
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 bg-zinc-400 rounded-full flex items-center justify-center text-xs text-black font-medium">이</div>
                  <span className="text-zinc-300 text-xs">이수현 (팀원)</span>
                </div>
              </td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center text-xs text-white">A</div>
                  <span className="font-mono text-blue-400 text-xs">AGT-002</span>
                  <span className="text-zinc-500 text-xs">뉴스스캐너</span>
                </div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-green-500/10 text-green-400">조회 · 리포트</span></td>
              <td className="px-4 py-3 text-xs text-zinc-400">4시간</td>
              <td className="px-4 py-3 text-xs text-zinc-400">리서치팀</td>
              <td className="px-4 py-3"><button className="text-xs text-blue-400 hover:text-blue-300">편집</button></td>
            </tr>
            <tr className="hover:bg-zinc-800/30 bg-yellow-500/5">
              <td className="px-4 py-3">
                <div className="text-xs text-yellow-500">— (없음)</div>
              </td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 bg-yellow-500 rounded-full flex items-center justify-center text-xs text-black">A</div>
                  <span className="font-mono text-yellow-400 text-xs">AGT-031</span>
                  <span className="text-zinc-500 text-xs">고아 에이전트</span>
                </div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-yellow-500/10 text-yellow-400">미배정</span></td>
              <td className="px-4 py-3 text-xs text-yellow-400">단절</td>
              <td className="px-4 py-3 text-xs text-zinc-400">미배정</td>
              <td className="px-4 py-3"><button className="text-xs text-yellow-400 hover:text-yellow-300">보고선 지정</button></td>
            </tr>
          </tbody>
        </table>
      </div>
