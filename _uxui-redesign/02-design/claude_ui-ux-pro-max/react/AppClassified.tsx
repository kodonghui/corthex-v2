...
function AppClassified() {
  return (
    <>      
      ...
      {/* API: GET /api/classified, GET /api/classified/:id (requires auth level 3) */}
  {/* Sidebar */}
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/app/classified" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>기밀문서</a>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">기밀문서</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">보안 등급 문서 관리</span>
      <div className="ml-auto flex items-center gap-2">
        <button className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-medium px-3 py-1.5 rounded-md transition-colors cursor-pointer">+ 기밀 문서 등록</button>
      </div>
    </div>
    <div className="flex-1 p-6">
      {/* Access warning */}
      <div className="flex items-center gap-3 px-4 py-3 bg-red-500/10 border border-red-500/20 rounded-lg mb-6">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
        <div>
          <span className="text-sm font-medium text-red-300">기밀 문서 열람 구역</span>
          <span className="text-xs text-red-400/70 ml-2">모든 접근이 기록됩니다. 권한 없는 열람은 보안 정책 위반입니다.</span>
        </div>
      </div>

      {/* Classification legend */}
      <div className="flex items-center gap-3 mb-5">
        <span className="px-2 py-0.5 rounded text-xs bg-red-500/20 text-red-300 border border-red-500/30">기밀</span>
        <span className="px-2 py-0.5 rounded text-xs bg-orange-500/20 text-orange-300 border border-orange-500/30">대외비</span>
        <span className="px-2 py-0.5 rounded text-xs bg-zinc-800 text-zinc-400 border border-zinc-700">일반</span>
        <div className="ml-auto">
          <input className="bg-zinc-900 border border-zinc-700 text-zinc-300 text-xs px-3 py-1.5 rounded-md placeholder-zinc-600 focus:outline-none focus:border-blue-500 transition-colors" placeholder="문서 검색..." />
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
        <table className="w-full">
          <thead><tr className="border-b border-zinc-800">
            <th className="text-left py-3 px-4 text-xs font-medium text-zinc-500">문서명</th>
            <th className="text-left py-3 px-4 text-xs font-medium text-zinc-500">분류</th>
            <th className="text-left py-3 px-4 text-xs font-medium text-zinc-500">생성자</th>
            <th className="text-left py-3 px-4 text-xs font-medium text-zinc-500">생성일</th>
            <th className="text-left py-3 px-4 text-xs font-medium text-zinc-500">접근자</th>
          ...
          <tbody>
            <tr className="border-b border-zinc-800/50 hover:bg-zinc-800/20 cursor-pointer transition-colors">
              <td className="py-3 px-4">
                <div className="flex items-center gap-2">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                  <span className="text-sm text-zinc-300">2026 전략 투자 포트폴리오 계획</span>
                </div>
              </td>
              <td className="py-3 px-4"><span className="px-2 py-0.5 rounded text-xs bg-red-500/20 text-red-300">기밀</span></td>
              <td className="py-3 px-4 text-xs text-zinc-400">김대표</td>
              <td className="py-3 px-4 text-xs font-mono text-zinc-600">2026-01-15</td>
              <td className="py-3 px-4 text-xs text-zinc-500">김대표, CIO봇</td>
              <td className="py-3 px-4">
                <button className="text-xs text-blue-400 hover:text-blue-300 transition-colors cursor-pointer">접근 요청</button>
              </td>
            </tr>
            <tr className="border-b border-zinc-800/50 hover:bg-zinc-800/20 cursor-pointer transition-colors">
              <td className="py-3 px-4">
                <div className="flex items-center gap-2">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="2"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                  <span className="text-sm text-zinc-300">핵심 알고리즘 소스코드 (트레이딩)</span>
                </div>
              </td>
              <td className="py-3 px-4"><span className="px-2 py-0.5 rounded text-xs bg-orange-500/20 text-orange-300">대외비</span></td>
              <td className="py-3 px-4 text-xs text-zinc-400">기술팀장</td>
              <td className="py-3 px-4 text-xs font-mono text-zinc-600">2026-02-03</td>
              <td className="py-3 px-4 text-xs text-zinc-500">김대표, 기술팀</td>
              <td className="py-3 px-4">
                <button className="text-xs text-blue-400 hover:text-blue-300 transition-colors cursor-pointer">접근 요청</button>
              </td>
            </tr>
            <tr className="border-b border-zinc-800/50 hover:bg-zinc-800/20 cursor-pointer transition-colors">
              <td className="py-3 px-4">
                <div className="flex items-center gap-2">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="2"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                  <span className="text-sm text-zinc-300">M&A 검토 보고서 — 파트너사 실사</span>
                </div>
              </td>
              <td className="py-3 px-4"><span className="px-2 py-0.5 rounded text-xs bg-orange-500/20 text-orange-300">대외비</span></td>
              <td className="py-3 px-4 text-xs text-zinc-400">비서실장</td>
              <td className="py-3 px-4 text-xs font-mono text-zinc-600">2026-03-01</td>
              <td className="py-3 px-4 text-xs text-zinc-500">김대표</td>
              <td className="py-3 px-4">
                <button className="text-xs text-blue-400 hover:text-blue-300 transition-colors cursor-pointer">접근 승인 완료</button>
              </td>
            </tr>
            <tr className="hover:bg-zinc-800/20 cursor-pointer transition-colors">
              <td className="py-3 px-4">
                <div className="flex items-center gap-2">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#71717a" strokeWidth="2"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                  <span className="text-sm text-zinc-300">직원 급여 현황 2026 Q1</span>
                </div>
              </td>
              <td className="py-3 px-4"><span className="px-2 py-0.5 rounded text-xs bg-zinc-800 text-zinc-400">일반</span></td>
              <td className="py-3 px-4 text-xs text-zinc-400">박매니저</td>
              <td className="py-3 px-4 text-xs font-mono text-zinc-600">2026-03-05</td>
              <td className="py-3 px-4 text-xs text-zinc-500">김대표, 박매니저</td>
              <td className="py-3 px-4">
                <button className="text-xs text-green-400 hover:text-green-300 transition-colors cursor-pointer">열람</button>
              </td>
            </tr>
          </tbody>
        </table>
