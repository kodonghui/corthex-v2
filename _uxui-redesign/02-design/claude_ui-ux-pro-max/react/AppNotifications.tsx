...
function AppNotifications() {
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">알림</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">시스템 & 에이전트 알림</span>
    ...
    <div className="flex-1 p-6">
      ...
      <div className="flex items-center gap-1 mb-5 border-b border-zinc-800">
        <button className="px-4 py-2 text-sm font-medium text-zinc-50 border-b-2 border-blue-500 -mb-px cursor-pointer">전체 (11)</button>
        <button className="px-4 py-2 text-sm font-medium text-zinc-500 hover:text-zinc-300 -mb-px cursor-pointer transition-colors">읽지않음 <span className="ml-1 text-xs text-red-400">4</span></button>
        <button className="px-4 py-2 text-sm font-medium text-zinc-500 hover:text-zinc-300 -mb-px cursor-pointer transition-colors">시스템</button>
        <button className="px-4 py-2 text-sm font-medium text-zinc-500 hover:text-zinc-300 -mb-px cursor-pointer transition-colors">에이전트</button>
        <div className="ml-auto pb-2">
          <button className="text-xs text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer">모두 읽음 처리</button>
        </div>
      </div>

      <div className="space-y-2 max-w-2xl">
        {/* Unread: agent complete */}
        <div className="flex items-start gap-3 p-4 bg-zinc-900 border border-blue-500/20 rounded-lg relative">
          <span className="absolute top-4 right-4 w-2 h-2 rounded-full bg-blue-400"></span>
          <div className="w-8 h-8 bg-green-500/10 border border-green-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </div>
          <div className="flex-1">
            <div className="text-sm font-medium text-zinc-50">작업 완료 — 삼성전자 분석</div>
            <p className="text-xs text-zinc-400 mt-0.5">전략분석가가 삼성전자 Q1 2026 분석을 완료했습니다. 리포트가 보고서 탭에 저장되었습니다.</p>
            <div className="flex items-center gap-3 mt-2">
              <a className="text-xs text-blue-400 hover:text-blue-300 transition-colors cursor-pointer">보고서 보기</a>
              <span className="text-xs text-zinc-600">14분 전</span>
            </div>
          </div>
        </div>

        {/* Unread: budget warning */}
        <div className="flex items-start gap-3 p-4 bg-zinc-900 border border-yellow-500/20 rounded-lg relative">
          <span className="absolute top-4 right-4 w-2 h-2 rounded-full bg-yellow-400"></span>
          ...
          <div className="flex-1">
            <div className="text-sm font-medium text-zinc-50">예산 경고 — 이번 달 78% 소진</div>
            <p className="text-xs text-zinc-400 mt-0.5">전체 CLI 토큰 예산 $30.00 중 $23.40 사용. 이번 달 21일 남았습니다.</p>
            <div className="flex items-center gap-2 mt-2">
              <button className="text-xs text-blue-400 hover:text-blue-300 transition-colors cursor-pointer">예산 조정</button>
              <span className="text-xs text-zinc-600">1시간 전</span>
            </div>
          </div>
        </div>

        {/* Unread: quality gate rework complete */}
        <div className="flex items-start gap-3 p-4 bg-zinc-900 border border-blue-500/20 rounded-lg relative">
          <span className="absolute top-4 right-4 w-2 h-2 rounded-full bg-blue-400"></span>
          <div className="w-8 h-8 bg-orange-500/10 border border-orange-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="2"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
          </div>
          <div className="flex-1">
            <div className="text-sm font-medium text-zinc-50">품질 게이트 → 재작업 완료</div>
            <p className="text-xs text-zinc-400 mt-0.5">전략분석가의 초안이 환각 탐지로 반려됐으나, 재작업 후 품질 게이트를 통과했습니다.</p>
            <span className="text-xs text-zinc-600 mt-1 block">2시간 전</span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-zinc-900 border border-blue-500/20 rounded-lg relative">
          ...
          <div className="w-8 h-8 bg-blue-500/10 border border-blue-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="2"><path d="M18 20V10M12 20V4M6 20v-6"/></svg>
          </div>
          <div className="flex-1">
            <div className="text-sm font-medium text-zinc-50">승인 요청 — 링크드인 아티클 발행</div>
            <p className="text-xs text-zinc-400 mt-0.5">마케팅봇이 작성한 링크드인 아티클 발행 승인이 필요합니다.</p>
            <div className="flex items-center gap-2 mt-2">
              <button className="text-xs bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-md transition-colors cursor-pointer">승인</button>
              <button className="text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1 rounded-md transition-colors cursor-pointer">거절</button>
              <span className="text-xs text-zinc-600 ml-2">3시간 전</span>
            </div>
          </div>
        </div>

        {/* Read notifications */}
        <div className="flex items-start gap-3 p-4 bg-zinc-900 border border-zinc-800 rounded-lg opacity-60">
          ...
          <div className="flex-1">
            <div className="text-sm font-medium text-zinc-300">오류 — SNS봇 트위터 인증 실패</div>
            <p className="text-xs text-zinc-500 mt-0.5">Twitter API OAuth 토큰 만료. 크리덴셜 갱신 필요.</p>
            <span className="text-xs text-zinc-600 mt-1 block">어제 14:30</span>
          </div>
        </div>

        <div className="flex items-start gap-3 p-4 bg-zinc-900 border border-zinc-800 rounded-lg opacity-60">
          <div className="w-8 h-8 bg-green-500/10 border border-green-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><path d="M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>
          </div>
          <div className="flex-1">
            <div className="text-sm font-medium text-zinc-300">새 팀원 초대 — 이대리</div>
            <p className="text-xs text-zinc-500 mt-0.5">이대리(lee@company.com)가 초대를 수락했습니다.</p>
            <span className="text-xs text-zinc-600 mt-1 block">3일 전</span>
          </div>
        </div>
      </div>
    </div>
