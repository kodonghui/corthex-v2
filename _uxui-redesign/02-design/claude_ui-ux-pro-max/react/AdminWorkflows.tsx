...
{/* GET /api/admin/workflows — returns workflow list with trigger, agents, run_count, last_run, status */}
  {/* Sidebar */}
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/admin/workflows" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>워크플로우</a>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">워크플로우</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">14개 자동화 파이프라인</span>
      <div className="ml-auto flex items-center gap-2">
        <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-md">+ 워크플로우 생성</button>
      </div>
    </div>
    <div className="flex-1 p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">등록 워크플로우</div>
          <div className="text-2xl font-semibold font-mono">14</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">활성</div>
          <div className="text-2xl font-semibold font-mono text-green-400">9</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">오늘 실행</div>
          <div className="text-2xl font-semibold font-mono">47</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">실패율</div>
          <div className="text-2xl font-semibold font-mono text-green-400">2.1%</div>
        </div>
      </div>

      <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
          <span className="text-sm font-medium">워크플로우 목록</span>
          <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-md">+ 워크플로우 생성</button>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-800">
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">트리거</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">참여 에이전트</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">오늘 실행</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500">마지막 실행</th>
          ...
          <tbody className="divide-y divide-zinc-800">
            <tr className="hover:bg-zinc-800/30">
              <td className="px-4 py-3">
                <div className="text-zinc-200">장전 시장 브리핑</div>
                <div className="text-xs text-zinc-600">AI 전략실 전체 배포</div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-blue-500/10 text-blue-400 font-mono">cron: 08:30</span></td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">AGT-001, AGT-002, AGT-004</td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">1</td>
              <td className="px-4 py-3 text-xs text-zinc-500 font-mono">08:31</td>
              ...
              <td className="px-4 py-3 flex gap-2">
                ...
                <button className="text-xs text-zinc-500 hover:text-zinc-400">중지</button>
              </td>
            </tr>
            <tr className="hover:bg-zinc-800/30">
              <td className="px-4 py-3">
                <div className="text-zinc-200">일일 성과 보고서</div>
                <div className="text-xs text-zinc-600">전략실 → 김대표</div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-blue-500/10 text-blue-400 font-mono">cron: 17:00</span></td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">AGT-001, AGT-007</td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">1</td>
              <td className="px-4 py-3 text-xs text-zinc-500 font-mono">어제 17:01</td>
              <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs bg-green-500/10 text-green-400">활성</span></td>
              <td className="px-4 py-3 flex gap-2">
                <button className="text-xs text-blue-400 hover:text-blue-300">편집</button>
                <button className="text-xs text-zinc-500 hover:text-zinc-400">중지</button>
            ...
            <tr className="hover:bg-zinc-800/30">
              <td className="px-4 py-3">
                <div className="text-zinc-200">리스크 경보 → 긴급 청산</div>
                <div className="text-xs text-zinc-600">MDD -5% 도달 시</div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-red-500/10 text-red-400 font-mono">event: risk_alert</span></td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">AGT-003, AGT-001</td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">0</td>
              <td className="px-4 py-3 text-xs text-zinc-500 font-mono">7일 전</td>
              <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs bg-green-500/10 text-green-400">대기</span></td>
              <td className="px-4 py-3 flex gap-2">
                <button className="text-xs text-blue-400 hover:text-blue-300">편집</button>
                <button className="text-xs text-zinc-500 hover:text-zinc-400">중지</button>
              </td>
            ...
            <tr className="hover:bg-zinc-800/30">
              <td className="px-4 py-3">
                <div className="text-zinc-200">뉴스 이상 감지 → 리포트</div>
                <div className="text-xs text-zinc-600">급등 뉴스 감지 시</div>
              </td>
              <td className="px-4 py-3"><span className="px-1.5 py-0.5 rounded text-xs bg-purple-500/10 text-purple-400 font-mono">event: news_spike</span></td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">AGT-002, AGT-004</td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">14</td>
              <td className="px-4 py-3 text-xs text-zinc-500 font-mono">11분 전</td>
              <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs bg-green-500/10 text-green-400">활성</span></td>
              <td className="px-4 py-3 flex gap-2">
                <button className="text-xs text-blue-400 hover:text-blue-300">편집</button>
                <button className="text-xs text-zinc-500 hover:text-zinc-400">중지</button>
