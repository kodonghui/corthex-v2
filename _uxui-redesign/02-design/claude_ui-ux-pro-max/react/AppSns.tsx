...
function AppSns() {
  return (
    <>      
      ...
      {/* API: GET /api/sns/accounts, GET /api/sns/posts, POST /api/sns/posts */}
  {/* Sidebar */}
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/app/sns" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/></svg>SNS</a>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">SNS</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">소셜 미디어 발행 관리</span>
      <div className="ml-auto flex items-center gap-2">
        <button className="bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium px-3 py-1.5 rounded-md transition-colors cursor-pointer">+ 포스팅 예약</button>
      </div>
    </div>
        ...
        <div className="col-span-2 space-y-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
            <div className="px-4 py-3 border-b border-zinc-800">
              <h3 className="text-sm font-semibold text-zinc-50">연결된 계정</h3>
            </div>
            <div className="divide-y divide-zinc-800/50">
              <div className="flex items-center gap-4 px-4 py-3">
                <div className="w-8 h-8 bg-zinc-800 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold text-zinc-300">X</div>
                <div className="flex-1">
                  <div className="text-sm font-medium text-zinc-300">@corthex_ai</div>
                  <div className="text-xs text-zinc-600">Twitter / X · 팔로워 12,847</div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-xs bg-green-500/10 text-green-400">연결됨</span>
              </div>
              <div className="flex items-center gap-4 px-4 py-3">
                <div className="w-8 h-8 bg-blue-600/20 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold text-blue-400">in</div>
                <div className="flex-1">
                  <div className="text-sm font-medium text-zinc-300">CORTHEX Inc.</div>
                  <div className="text-xs text-zinc-600">LinkedIn · 팔로워 3,241</div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-xs bg-green-500/10 text-green-400">연결됨</span>
              </div>
              <div className="flex items-center gap-4 px-4 py-3">
                <div className="w-8 h-8 bg-pink-600/20 rounded-full flex items-center justify-center flex-shrink-0">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#ec4899" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r=".5" fill="#ec4899"/></svg>
                </div>
                <div className="flex-1">
                  <div className="text-sm font-medium text-zinc-300">@corthex.ai</div>
                  <div className="text-xs text-zinc-600">Instagram · 팔로워 5,632</div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-xs bg-green-500/10 text-green-400">연결됨</span>
              </div>
              <div className="flex items-center gap-4 px-4 py-3">
                <div className="w-8 h-8 bg-green-600/20 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold text-green-400">N</div>
                <div className="flex-1">
                  <div className="text-sm font-medium text-zinc-300">CORTHEX 블로그</div>
                  <div className="text-xs text-zinc-600">Naver Blog · 방문자 일 320</div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-xs bg-red-500/10 text-red-400">인증 오류</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
              <h3 className="text-sm font-semibold text-zinc-50">예약 발행</h3>
              <button className="text-xs text-blue-400 hover:text-blue-300 cursor-pointer transition-colors">+ 포스팅 예약</button>
            </div>
            <div className="divide-y divide-zinc-800/50">
              <div className="px-4 py-3 hover:bg-zinc-800/30 cursor-pointer transition-colors">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs text-zinc-600 bg-zinc-800 px-1.5 py-0.5 rounded font-bold">X</span>
                  <span className="text-xs text-zinc-600">2026-03-10 18:00</span>
                  <span className="px-2 py-0.5 rounded-full text-xs bg-yellow-500/10 text-yellow-400 ml-auto">예약됨</span>
                </div>
                <p className="text-sm text-zinc-300">[AI 조직 운영 인사이트] 2026년 1분기, CORTHEX의 AI 에이전트 팀이 달성한 성과를 공개합니다. 비용 43% 절감, 작업 효율 2.8배 향상...</p>
                <div className="text-xs text-zinc-600 mt-1">SNS봇 · 자동 생성</div>
              </div>
              <div className="px-4 py-3 hover:bg-zinc-800/30 cursor-pointer transition-colors">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs text-zinc-600 bg-pink-900/30 px-1.5 py-0.5 rounded text-pink-400">IG</span>
                  <span className="text-xs text-zinc-600">2026-03-11 10:00</span>
                  <span className="px-2 py-0.5 rounded-full text-xs bg-yellow-500/10 text-yellow-400 ml-auto">예약됨</span>
                </div>
                <p className="text-sm text-zinc-300">AI 에이전트와 함께하는 스마트 투자 전략 인포그래픽 #AI투자 #CORTHEX</p>
                <div className="text-xs text-zinc-600 mt-1">마케팅봇 · 수동 편집됨</div>
              </div>
              <div className="px-4 py-3 hover:bg-zinc-800/30 cursor-pointer transition-colors">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs text-blue-400 bg-blue-900/20 px-1.5 py-0.5 rounded font-bold">in</span>
                  <span className="text-xs text-zinc-600">2026-03-12 09:00</span>
                  <span className="px-2 py-0.5 rounded-full text-xs bg-yellow-500/10 text-yellow-400 ml-auto">승인 대기</span>
                </div>
                <p className="text-sm text-zinc-300">B2B SaaS에서 AI 에이전트 조직을 구성하는 방법 — 실무 가이드 링크드인 아티클</p>
                <div className="text-xs text-zinc-600 mt-1">마케팅봇 · 김대표 승인 필요</div>
              </div>
            </div>
          </div>
        </div>

        {/* Right: Recent published */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg flex flex-col">
          <div className="px-4 py-3 border-b border-zinc-800 flex-shrink-0">
            <h3 className="text-sm font-semibold text-zinc-50">최근 발행</h3>
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-zinc-800/50">
            <div className="px-4 py-3 hover:bg-zinc-800/30 cursor-pointer transition-colors">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-xs bg-zinc-800 px-1 py-0.5 rounded font-bold text-zinc-300">X</span>
                <span className="text-xs text-zinc-600">3시간 전</span>
              </div>
              <p className="text-xs text-zinc-400 leading-relaxed">AI 에이전트 조직 운영 팁 #1...</p>
            ...
            <div className="px-4 py-3 hover:bg-zinc-800/30 cursor-pointer transition-colors">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-xs bg-pink-900/30 px-1 py-0.5 rounded text-pink-400">IG</span>
                <span className="text-xs text-zinc-600">어제</span>
              </div>
              <p className="text-xs text-zinc-400 leading-relaxed">CORTHEX 팀 소개 인포그래픽...</p>
              <div className="flex items-center gap-3 mt-2 text-xs text-zinc-600">
                <span>♥ 892</span>
                <span>💬 34</span>
              </div>
            ...
            <div className="px-4 py-3 hover:bg-zinc-800/30 cursor-pointer transition-colors">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-xs bg-blue-900/20 px-1 py-0.5 rounded text-blue-400">in</span>
                <span className="text-xs text-zinc-600">3일 전</span>
              </div>
              <p className="text-xs text-zinc-400 leading-relaxed">AI 조직 ROI 분석 아티클...</p>
              <div className="flex items-center gap-3 mt-2 text-xs text-zinc-600">
                <span>♥ 421</span>
                <span>💬 67</span>
              </div>
