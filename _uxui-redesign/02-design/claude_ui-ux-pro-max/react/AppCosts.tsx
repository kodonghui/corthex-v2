...
function AppCosts() {
  ...
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/app/costs" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>비용</a>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">비용</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">CLI 토큰 사용 비용 추적</span>
      <div className="ml-auto flex items-center gap-2">
        <button className="text-xs text-zinc-400 bg-zinc-900 border border-zinc-700 hover:bg-zinc-800 px-3 py-1.5 rounded-md transition-colors cursor-pointer">예산 설정</button>
      </div>
    </div>
    <div className="flex-1 p-6">
      {/* Period selector + KPI */}
      <div className="flex items-center gap-2 mb-5">
        <button className="px-3 py-1.5 text-xs font-medium text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 rounded-md transition-colors cursor-pointer">일</button>
        <button className="px-3 py-1.5 text-xs font-medium text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 rounded-md transition-colors cursor-pointer">주</button>
        <button className="px-3 py-1.5 text-xs font-medium bg-zinc-800 text-zinc-50 rounded-md cursor-pointer">월</button>
        <span className="text-xs text-zinc-600 ml-2">2026년 3월</span>
      </div>

      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-2">이번 달 총 비용</div>
          <div className="text-2xl font-semibold font-mono text-zinc-50">$23.40</div>
          <div className="text-xs text-zinc-500 mt-1">₩31,320 (환율 1,338원)</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-2">Batch API 절감</div>
          <div className="text-2xl font-semibold font-mono text-green-400">$4.20</div>
          <div className="text-xs text-green-400/70 mt-1">일반 대비 15.2% 절약</div>
        </div>
      </div>

      {/* Bar chart by dept */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 mb-6">
        <h3 className="text-sm font-semibold text-zinc-50 mb-4">부서별 비용 (3월)</h3>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-zinc-800">
          <h3 className="text-sm font-semibold text-zinc-50">에이전트별 비용 상세</h3>
        </div>
        <table className="w-full">
          <thead><tr className="border-b border-zinc-800">
            <th className="text-left py-2.5 px-4 text-xs font-medium text-zinc-500">에이전트</th>
            <th className="text-left py-2.5 px-4 text-xs font-medium text-zinc-500">모델</th>
            <th className="text-right py-2.5 px-4 text-xs font-medium text-zinc-500">입력 토큰</th>
            <th className="text-right py-2.5 px-4 text-xs font-medium text-zinc-500">출력 토큰</th>
            <th className="text-right py-2.5 px-4 text-xs font-medium text-zinc-500">비용</th>
          </tr></thead>
          <tbody>
            <tr className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
              ...
              <td className="py-2.5 px-4 text-xs font-mono text-zinc-500">claude-sonnet-4-6</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">1,234,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">287,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-300">$4.21</td>
            </tr>
            <tr className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
              <td className="py-2.5 px-4 text-sm text-zinc-300">CIO봇</td>
              <td className="py-2.5 px-4 text-xs font-mono text-zinc-500">claude-sonnet-4-6</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">2,150,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">420,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-300">$7.12</td>
            </tr>
            <tr className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
              <td className="py-2.5 px-4 text-sm text-zinc-300">전략분석가</td>
              <td className="py-2.5 px-4 text-xs font-mono text-zinc-500">claude-haiku-3-5</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">3,421,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">891,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-300">$1.07</td>
            </tr>
            <tr className="hover:bg-zinc-800/20 transition-colors">
              <td className="py-2.5 px-4 text-sm text-zinc-300">마케팅봇</td>
              <td className="py-2.5 px-4 text-xs font-mono text-zinc-500">claude-haiku-3-5</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">5,780,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-400">1,234,000</td>
              <td className="py-2.5 px-4 text-right text-xs font-mono text-zinc-300">$1.87</td>
            </tr>
          </tbody>
        </table>
      </div>
