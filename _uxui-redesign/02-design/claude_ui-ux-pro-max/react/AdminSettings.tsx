...
function AdminSettings() {
  return (
    <>      
      ...
      {/* GET/PUT /api/admin/settings — returns workspace config: timezone, language, cost_limits */}
  {/* Sidebar */}
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/admin/settings" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065zM15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>설정</a>
    </nav>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">시스템 설정</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">워크스페이스 전역 설정</span>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <div>
          <nav className="space-y-0.5">
            <button className="w-full text-left px-3 py-2 rounded-md text-sm bg-zinc-800 text-zinc-50">일반</button>
            <button className="w-full text-left px-3 py-2 rounded-md text-sm text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50">보안</button>
            <button className="w-full text-left px-3 py-2 rounded-md text-sm text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50">알림</button>
            <button className="w-full text-left px-3 py-2 rounded-md text-sm text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50">인테그레이션</button>
            <button className="w-full text-left px-3 py-2 rounded-md text-sm text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50">청구</button>
          </nav>
        </div>

        <div className="col-span-3 space-y-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-5">
            <div className="text-sm font-semibold text-zinc-200 mb-4">일반 설정</div>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-zinc-500 mb-1.5">워크스페이스 이름</label>
                  <input type="text" value="CORTHEX AI (주)" className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-zinc-500" />
                </div>
                <div>
                  <label className="block text-xs text-zinc-500 mb-1.5">사업자 등록번호</label>
                  <input type="text" value="123-45-67890" className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-zinc-500" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-zinc-500 mb-1.5">기본 시간대</label>
                <select className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-zinc-500">
              ...
              <div>
                <label className="block text-xs text-zinc-500 mb-1.5">기본 언어</label>
                <select className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-zinc-500">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-5">
            <div className="text-sm font-semibold text-zinc-200 mb-4">CLI 토큰 전역 설정</div>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-zinc-500 mb-1.5">월 전체 한도</label>
                  <input type="text" value="8,000,000" className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-300 font-mono focus:outline-none focus:border-zinc-500" />
                </div>
                <div>
                  <label className="block text-xs text-zinc-500 mb-1.5">경보 임계치</label>
                  <input type="text" value="80%" className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-300 font-mono focus:outline-none focus:border-zinc-500" />
                </div>
              </div>
              <div className="flex items-center justify-between p-3 bg-zinc-800/50 rounded-md">
                <div>
                  <div className="text-sm text-zinc-300">한도 초과 시 에이전트 자동 중지</div>
                  <div className="text-xs text-zinc-600 mt-0.5">월 한도 100% 도달 시 모든 에이전트 일시 중지</div>
                </div>
                <div className="w-10 h-5 bg-blue-600 rounded-full flex items-center relative cursor-pointer">
          ...
          <div className="flex justify-end gap-3">
            <button className="text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-4 py-2 rounded-md">취소</button>
            <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-md">변경사항 저장</button>
          </div>
        </div>
      </div>
