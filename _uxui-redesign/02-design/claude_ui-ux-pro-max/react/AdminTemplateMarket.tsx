...
function AdminTemplateMarket() {
  return (
    <>      
      ...
      {/* GET /api/admin/template-market — returns marketplace listings with install_count, rating */}
  {/* Sidebar */}
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/admin/template-market" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>템플릿마켓</a>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">템플릿 마켓</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">커뮤니티 Soul 템플릿</span>
      <div className="ml-auto flex items-center gap-2">
        <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-md">내 템플릿 공개</button>
      </div>
    </div>
    <div className="flex-1 p-6">
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="템플릿 검색..." className="w-full bg-zinc-900 border border-zinc-800 rounded-md pl-9 pr-4 py-1.5 text-sm text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-zinc-600" />
        </div>
      ...
      {/* Featured */}
      <div className="bg-blue-500/5 border border-blue-500/20 rounded-lg p-4 mb-5 flex items-center gap-4">
        <div className="w-12 h-12 bg-blue-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm font-semibold text-zinc-200">이달의 추천: KIS 풀 퀀트 패키지</span>
            <span className="px-2 py-0.5 rounded text-xs bg-blue-500/20 text-blue-300">에디터 픽</span>
          </div>
          <p className="text-xs text-zinc-500">KIS API 연동 · 자동 주문 · 리스크 제어 · 성과 리포트까지 완결된 퀀트 운용 패키지. 설치 5분.</p>
        </div>
        <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-md flex-shrink-0">무료 설치</button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 hover:border-zinc-700 transition-colors cursor-pointer">
          <div className="flex items-start justify-between mb-3">
            <div className="w-9 h-9 bg-green-500/20 rounded-lg flex items-center justify-center">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><path d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
            </div>
            <div className="text-right">
              <div className="text-xs text-yellow-400">★★★★★</div>
              <div className="text-xs text-zinc-600">4.9 (124)</div>
            </div>
          </div>
          <div className="text-sm font-medium text-zinc-200 mb-1">모멘텀 트레이더</div>
          <div className="text-xs text-zinc-500 mb-3">모멘텀 지표 기반 자동 매매. 일 최대 30회 거래.</div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-zinc-600 font-mono">2,847 설치</span>
            <button className="text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1 rounded-md">설치</button>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 hover:border-zinc-700 transition-colors cursor-pointer">
          <div className="flex items-start justify-between mb-3">
            <div className="w-9 h-9 bg-purple-500/20 rounded-lg flex items-center justify-center">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2"><path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5"/></svg>
            </div>
            <div className="text-right">
              <div className="text-xs text-yellow-400">★★★★☆</div>
              <div className="text-xs text-zinc-600">4.6 (89)</div>
            </div>
          </div>
          <div className="text-sm font-medium text-zinc-200 mb-1">뉴스 감성 분석기</div>
          <div className="text-xs text-zinc-500 mb-3">실시간 경제 뉴스 수집 및 GPT 감성 분석.</div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-zinc-600 font-mono">1,392 설치</span>
            <button className="text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1 rounded-md">설치</button>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 hover:border-zinc-700 transition-colors cursor-pointer">
          <div className="flex items-start justify-between mb-3">
            ...
            <div className="text-right">
              <div className="text-xs text-yellow-400">★★★★★</div>
              <div className="text-xs text-zinc-600">4.8 (211)</div>
            </div>
          </div>
          <div className="text-sm font-medium text-zinc-200 mb-1">포트폴리오 가드</div>
          <div className="text-xs text-zinc-500 mb-3">드로다운 -5% 이상 시 자동 손절 및 알림.</div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-zinc-600 font-mono">3,021 설치</span>
            <button className="text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1 rounded-md">설치</button>
          </div>
        </div>
      </div>
