...
{/* API: GET /api/me, PATCH /api/me, POST /api/me/cli-token */}
  {/* Sidebar */}
  <aside className="w-60 min-w-[240px] bg-zinc-900 border-r border-zinc-800 flex flex-col fixed h-full z-20">
    ...
    <nav className="flex-1 overflow-y-auto p-2 space-y-0.5">
      ...
      <a href="/app/settings" className="flex items-center gap-2.5 px-2 py-1.5 rounded-md bg-zinc-800 text-zinc-50 cursor-pointer text-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065zM15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>설정</a>
    </nav>
  ...
  <main className="ml-60 flex-1 flex flex-col min-h-screen overflow-y-auto">
    <div className="h-12 border-b border-zinc-800 flex items-center px-6 gap-3 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10 flex-shrink-0">
      <span className="text-sm font-medium text-zinc-50">설정</span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-500">프로필 & 기본 설정</span>
    ...
    <div className="flex-1 p-6">
      ...
      <div className="flex items-center gap-1 mb-6 border-b border-zinc-800">
        <button className="px-4 py-2 text-sm font-medium text-zinc-50 border-b-2 border-blue-500 -mb-px cursor-pointer">프로필</button>
        <button className="px-4 py-2 text-sm font-medium text-zinc-500 hover:text-zinc-300 -mb-px cursor-pointer transition-colors">알림</button>
        <button className="px-4 py-2 text-sm font-medium text-zinc-500 hover:text-zinc-300 -mb-px cursor-pointer transition-colors">보안</button>
        <button className="px-4 py-2 text-sm font-medium text-zinc-500 hover:text-zinc-300 -mb-px cursor-pointer transition-colors">외관</button>
      </div>

      <div className="max-w-lg space-y-6">
        {/* Avatar + name */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
          <h3 className="text-sm font-semibold text-zinc-50 mb-4">기본 정보</h3>
          <div className="flex items-center gap-4 mb-6">
            <div className="w-16 h-16 bg-zinc-700 rounded-full flex items-center justify-center text-2xl font-semibold text-zinc-300">김</div>
            <div>
              <button className="text-xs text-blue-400 hover:text-blue-300 transition-colors cursor-pointer block">아바타 변경</button>
              <p className="text-xs text-zinc-600 mt-1">JPG, PNG 최대 2MB</p>
            </div>
          </div>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">이름</label>
              <input className="w-full bg-zinc-900 border border-zinc-700 text-zinc-50 text-sm px-3 py-2 rounded-md focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" value="김대표" />
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">이메일</label>
              <input className="w-full bg-zinc-900 border border-zinc-700 text-zinc-50 text-sm px-3 py-2 rounded-md focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" value="ceo@corthex.ai" />
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">역할</label>
              <input className="w-full bg-zinc-800/50 border border-zinc-800 text-zinc-500 text-sm px-3 py-2 rounded-md cursor-not-allowed" value="CEO / 관리자" disabled />
            </div>
          </div>
        </div>
        ...
        {/* CLI Token */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
          <h3 className="text-sm font-semibold text-zinc-50 mb-1">CLI 토큰 설정</h3>
          <p className="text-xs text-zinc-500 mb-4">Claude CLI Max 구독 토큰을 등록하면 AI 에이전트가 이 토큰으로 실행됩니다.</p>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">CLI 토큰</label>
              <div className="flex gap-2">
                <input className="flex-1 bg-zinc-900 border border-zinc-700 text-zinc-400 text-sm px-3 py-2 rounded-md font-mono focus:outline-none focus:border-blue-500 transition-colors" type="password" value="sk-ant-cli-01-••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••" />
                <button className="text-xs text-zinc-500 hover:text-zinc-300 bg-zinc-800 hover:bg-zinc-700 px-3 py-2 rounded-md transition-colors cursor-pointer">표시</button>
              </div>
              <p className="text-xs text-zinc-600 mt-1">AES-256-GCM 암호화 저장</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">월 사용 한도</label>
              <div className="flex gap-2 items-center">
                <input className="w-24 bg-zinc-900 border border-zinc-700 text-zinc-50 text-sm px-3 py-2 rounded-md focus:outline-none focus:border-blue-500 transition-colors" value="30" />
                <span className="text-xs text-zinc-500">USD / 월</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Locale */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
          <h3 className="text-sm font-semibold text-zinc-50 mb-4">언어 & 지역</h3>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">언어</label>
              <select className="w-full bg-zinc-900 border border-zinc-700 text-zinc-300 text-sm px-3 py-2 rounded-md focus:outline-none focus:border-blue-500 transition-colors">
            ...
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">시간대</label>
              <select className="w-full bg-zinc-900 border border-zinc-700 text-zinc-300 text-sm px-3 py-2 rounded-md focus:outline-none focus:border-blue-500 transition-colors">
        ...
        <div className="flex justify-end">
          <button className="bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium px-6 py-2 rounded-md transition-colors cursor-pointer">변경사항 저장</button>
        </div>
      </div>
    </div>
  </main>
