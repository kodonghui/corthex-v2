...
### 오케스트레이터 프로토콜 (팀 에이전트 위임)
메인 대화는 **오케스트레이터** 역할만 하고, 각 BMAD 단계는 **팀 에이전트에게 위임**한다.
이렇게 하면 각 에이전트가 깨끗한 컨텍스트로 시작하여 품질 저하를 방지한다.

**스토리 1개 처리 흐름:**
```
메인(오케스트레이터)
  ├─ Agent 1: create-story → 스토리 파일 생성 → 완료 보고
  ├─ Agent 2: dev-story → 코드 구현 → 완료 보고
  ├─ Agent 3: TEA → 테스트 생성+실행 → 완료 보고
  ├─ Agent 4: QA → 기능 검증 → 완료 보고
  ├─ Agent 5: code-review → 리뷰+수정 → 완료 보고
  └─ 메인: 체크리스트 확인 → 커밋+푸시 → 다음 스토리
```

**팀 에이전트 프롬프트 규칙:**
- 각 에이전트에게 **스킬명 + 인자 + YOLO 모드 + 자동 실행** 지시를 명확히 전달
- BMAD 에이전트(QA 등)는 메뉴 표시 없이 바로 실행하라고 프롬프트에 명시
- code-review 이슈 발견 시 자동 수정 [1] 선택하라고 명시
- 에이전트가 돌아오면 결과 확인 후 다음 단계로 진행

**에픽 단위 운영:**
- 에픽 내 모든 스토리를 순차 처리 (스토리 간 의존성 있으므로)
- 에픽 완료 후 retrospective도 팀 에이전트에게 위임
- 컨텍스트가 길어지면 에픽 중간이라도 새 대화로 이어갈 것
