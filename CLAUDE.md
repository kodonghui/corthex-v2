...
### 컨텍스트 관리 규칙 (Method 1)
- **1 에픽 = 1 대화**: 에픽 하나가 끝나면 반드시 새 대화에서 다음 에픽을 시작할 것
- 한 대화 안에서 여러 에픽을 연속으로 하지 말 것 (컨텍스트 피로로 BMAD 준수율 하락)
- 에픽 중간에 대화가 길어지면 스토리 단위로 끊고 새 대화에서 이어갈 것

### 커밋 전 하드 체크리스트 (Method 2)
매 스토리 커밋 직전, 아래 5개 항목을 **텍스트로 출력**하고 전부 체크되어야만 커밋 가능:
```
[BMAD 체크리스트 — Story X-Y]
[ ] 1. create-story 스킬 호출 완료
[ ] 2. dev-story 스킬 호출 완료
[ ] 3. TEA (qa-generate-e2e-tests) 스킬 호출 완료
[ ] 4. QA (bmad-agent-bmm-qa) 에이전트 호출 완료
[ ] 5. code-review 스킬 호출 완료
```
