...
## QA 테스트 절대 규칙 — 거짓 보고 금지
- **"작동"이라고 보고하려면 반드시: (1) 해당 요소를 직접 클릭/입력 (2) 스크린샷 촬영 (3) 스크린샷에서 기대 결과 확인. 3개 중 하나라도 빠지면 "미수행"으로 기재.**
- **페이지가 로드된 것 ≠ 기능이 작동하는 것. 버튼을 직접 클릭하지 않고 "작동"이라고 적는 것은 거짓 보고.**
- **3~4개 섹션 단위로 끊어서 실행. 한 번에 15개 섹션 몰아서 하지 않음. 섹션 완료 후 사용자에게 보고하고 다음 진행.**
- **컨텍스트가 길어져서 집중도가 떨어지면 솔직하게 "여기까지 했습니다" 보고. 거짓 완료보다 정직한 미완료가 낫다.**
- **보고서에 스크린샷 ID 필수 기재: `작동 (ss_xxxx에서 확인)` 형식. 스크린샷 없는 "작동" = 거짓 보고.**
- Chrome 브라우저 자동화(claude-in-chrome)도 이 규칙을 동일하게 따름.
- 테스트 데이터 이름은 이전 테스트와 겹치지 않게 새로 생성.
