...
- [ ] Task 1: Sticky 배너 + 취소 버튼 (AC: #1, #3)
  - [ ] 변경 감지 시 상단 sticky 배너: "저장되지 않은 변경사항이 있습니다" + [저장] [취소]
  - [ ] [취소] 클릭 시 원래 서버 데이터로 롤백
- [ ] Task 2: 페이지 이탈 경고 (AC: #4)
  - [ ] `beforeunload` 이벤트로 브라우저 이탈 방지
  - [ ] React Router의 `useBlocker` 또는 `window.confirm`으로 내부 네비게이션 차단
- [ ] Task 3: Store 연동 + 토스트 (AC: #2, #5)
  - [ ] admin-store companyId 연동
  - [ ] 저장 성공 시 토스트
- [ ] Task 4: 테스트
  - [ ] 보고 라인 일괄 PUT API 테스트
  - [ ] 순환 보고 방지 (A→B→A) 서버 검증 테스트
