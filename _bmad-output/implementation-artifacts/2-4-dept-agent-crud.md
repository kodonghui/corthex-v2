...
- [x] Task 1: 부서 CRUD 보강 (AC: #1, #2, #6)
  - [x] 서버: 부서 생성 시 동일 회사 내 이름 중복 체크 → 409
  - [x] 서버: 부서 삭제 시 소속 에이전트 수 확인 → 있으면 409
  - [x] 프론트: admin-store companyId 연동
- [x] Task 2: 에이전트-직원 연결 UI (AC: #3, #4)
  - [x] 에이전트 생성/수정 폼에 "담당 직원" 드롭다운 (userId 선택)
  - [x] 서버: agents 테이블의 userId 업데이트 API 확인
  - [x] 직원 연결 시 해당 직원의 CLI 토큰 존재 여부 표시 (유/무 뱃지)
- [x] Task 3: 소울 편집 개선 (AC: #5)
  - [x] 에이전트 수정 시 소울 textarea → 더 넓은 편집 모달
  - [x] 마크다운 미리보기 (간단한 렌더링)
- [x] Task 4: 즉시 활성화 검증 (AC: #3)
  - [x] 에이전트 생성 후 status='offline' + isActive=true 기본값 확인
  - [x] 서버 재시작 없이 새 에이전트 API 조회 가능 확인 테스트
- [x] Task 5: 테스트
  - [x] 부서 중복 이름 409 테스트
  - [x] 부서 삭제 보호 409 테스트
  - [x] 에이전트 CRUD + userId 연결 테스트
...
- 부서 중복 이름 체크 (409)
- 부서 삭제 보호 (소속 에이전트 확인 409)
- 에이전트-직원 연결 UI (userId 드롭다운)
- admin-store companyId 연동

### File List
- packages/admin/src/pages/departments.tsx — store 연동 + 삭제 보호 UI
- packages/admin/src/pages/agents.tsx — userId 드롭다운 + 비서 토글
- packages/server/src/routes/admin/departments.ts — 중복/삭제 보호
- packages/server/src/routes/admin/agents.ts — userId 연결
