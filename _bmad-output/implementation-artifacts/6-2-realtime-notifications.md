...
- notifications.tsx 전면 교체: Tabs(알림 목록/설정) + 날짜 그룹 + 유형 아이콘 + 미읽음 인디케이터
- 전체/미확인만 필터 + 모두 읽음 버튼
- WS notifications 채널 구독 → React Query invalidation
- sidebar.tsx: 30초 폴링 뱃지 (unreadCount > 0 시 인디고 뱃지)
- notification-listener.tsx: 전역 WS 리스너 + /notifications 외 페이지에서 Toast
- layout.tsx: ToastProvider + NotificationListener 마운트

### File List
- packages/app/src/pages/notifications.tsx (REWRITTEN)
- packages/app/src/components/sidebar.tsx (MODIFIED — 뱃지 추가)
- packages/app/src/components/notification-listener.tsx (NEW)
- packages/app/src/components/layout.tsx (MODIFIED — Toast/Listener)
