...
- [ ] Task 1: 비서 역할 토글 UI (AC: #1, #2)
  - [ ] 에이전트 편집 모달/폼에 "비서 역할" 토글 스위치 추가
  - [ ] 에이전트 카드에 isSecretary=true면 비서 뱃지 (이미 있음, 확인)
  - [ ] 에이전트 생성 시에도 비서 역할 옵션 제공
- [ ] Task 2: 세션 강제 종료 API (AC: #3, #4)
  - [ ] `POST /api/admin/users/:id/terminate-session` 라우트 신규
  - [ ] sessions 테이블에서 해당 userId의 모든 활성 세션 삭제 또는 만료 처리
  - [ ] 활동 로그 기록: type='system', action='세션 강제 종료'
- [ ] Task 3: 세션 종료 UI (AC: #3, #5)
  - [ ] 직원 목록 테이블 행에 "세션 종료" 버튼 (빨간색, 활성 유저만)
  - [ ] 클릭 → 확인 모달: "{이름}의 모든 세션을 종료하시겠습니까? 즉시 로그아웃됩니다."
  - [ ] 성공 시 토스트: "세션이 종료되었습니다"
- [ ] Task 4: 테스트
  - [ ] 세션 종료 후 해당 유저 JWT 401 테스트
  - [ ] 비서 역할 토글 API 테스트
  - [ ] 활동 로그 기록 확인 테스트
