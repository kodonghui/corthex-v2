...
# Product Brief: CORTHEX v3 "OpenClaw"
...
CORTHEX v3 "OpenClaw"는 AI 에이전트들이 **개성을 갖고, 성장하며, 실제로 일하는 모습을 볼 수 있는** 엔터프라이즈 AI 조직 운영 플랫폼이다.

v2에서 검증된 인프라(485개 API 엔드포인트, 86개 DB 테이블, 71개 프론트엔드 페이지, 10,154개 테스트)를 기반으로 4가지 핵심 능력을 추가한다:

1. **OpenClaw 가상 사무실** — PixiJS 8 픽셀 캐릭터로 에이전트가 실시간으로 일하는 모습을 시각화 (CEO 앱 `/office`)
...
동시에 기존 테마의 색상 혼재(428곳), dead button, 메뉴 구조 불일치 문제를 해결하는 완전한 UXUI 리디자인을 수행한다. 디자인 도구: **Subframe**(메인) + `/kdh-uxui-redesign-full-auto-pipeline Phase 0`부터 새 테마 아키타입 결정.

**핵심 성과 목표** (정량 지표는 Step 4에서 정의): 에이전트 반복 오류율 감소, 워크플로우 설정 시간 단축, Big Five 성격 기반 응답 일관성 향상, Reflection 크론 비용 대비 태스크 품질 개선.
...
CORTHEX v2가 이미 구축한 485개 API, 68개 built-in 도구, 14개 WebSocket 채널, 6개 백그라운드 워커 위에 v3의 4가지 레이어를 얹는다 — 어떤 경쟁 솔루션도 이 규모의 검증된 엔터프라이즈 기반을 갖추고 있지 않다.
...
- **채택 전략: Option B — 기존 확장** (대체·병렬 아님)
  ...
  - 신규 `observations` 테이블만 추가 (raw 실행 로그 — 기존에 없는 계층)
  - 기존 v2 메모리 데이터 단절 없음

**3단계 파이프라인:**
- **관찰(Observation)**: 모든 실행 로그 → 신규 `observations` 테이블 (PostgreSQL)
- **반성(Reflection)**: 크론 주기적 실행(기본 일 1회) → 관찰 요약 → 기존 `agent_memories` 테이블에 `memoryType: 'reflection'`으로 저장 (pgvector 임베딩)
  - ⚠️ **비용 모델**: Reflection마다 LLM API 호출 발생. 에이전트 수 × 관찰량 × 빈도 = 비용 가변. Tier별 Reflection 한도 설정 필요 (상세 설계는 Step 4 Metrics에서 정의).
...
#### Layer 1: OpenClaw 가상 사무실 (투명성 해결) — 4번째 구현 (에셋 선행 필요)
...
- **픽셀 아트 UX 철학**: 픽셀 캐릭터는 장식이 아니라 **투명성 인터페이스**다. 텍스트 로그를 읽을 수 없는 CEO도 에이전트가 지금 무엇을 하는지 한눈에 파악한다. AI Town이 시뮬레이션인 반면, OpenClaw는 실제 `agent-loop.ts` 실행 로그를 실시간으로 픽셀 동작으로 변환한다 — 같은 엔진, 다른 창문.
- 기존 `agent-loop.ts` 실행 로그 읽기만 — 엔진 변경 없음
- WebSocket: 기존 14채널에 `/ws/office` 채널 1개 추가 (VPS: 번들 < 200KB gzipped)
...
5. **UXUI 완전 리셋**: 모든 버튼이 작동하고, 모든 색상이 의미를 갖고, AI 조직의 규모에 어울리는 인터페이스로 완전 재구축. 기존 428곳 색상 혼재와 dead button 문제를 패치 없이 근본 해결. 도구: Subframe(메인 디자인) + `/kdh-uxui-redesign-full-auto-pipeline Phase 0`에서 5개 아키타입 테마 중 선택. OpenClaw 픽셀 감성과 앱 전체 UI 테마의 접점은 Phase 0 테마 결정 후 확정.
...
| 접근 앱 | Admin 앱 (27개 페이지 v2 기준, v3 +1 예정) |
...
> **페이지 수 참고**: v2 총 71개 = Admin 27 + CEO 42 + Login 2. v3 신규 2페이지(/office, n8n 관리) 추가 예정 → **73개** 예상.
...
- Phase 0 디자인 게이팅: Subframe 기준 대비 구현 일치율 ≥ 95%
...
> **결정**: v3 "OpenClaw" 단일 릴리즈 — Phase 없음. 4개 레이어 전부 v3에 포함.

### Sprint 구현 순서 (Step 02 확정)

| Sprint | 레이어 | 내용 | 의존성 / 선행 조건 |
|--------|--------|------|--------|
| **Pre-Sprint** | **Phase 0** | 디자인 토큰 확정 (Subframe 아키타입 선택) | **Sprint 1 착수 전 완료 필수** — 미확정 시 전 Sprint UI 재작업 리스크 |
| 병행 | **Layer 0** | UXUI 완전 리셋 | 전 Sprint 인터리브. Sprint별 해당 기능 페이지 UXUI 병행. 중간 게이팅: Sprint 2 종료까지 ≥ 60% 미달 시 레드라인 검토 |
| Sprint 1 | **Layer 3** | Big Five 성격 시스템 | 독립 · 낮음. Phase 0 테마 확정 선행 필수 |
| Sprint 2 | **Layer 2** | n8n 워크플로우 연동 | 독립 · 중간 |
| Sprint 3 | **Layer 4** | 에이전트 메모리 3단계 | 복잡 · 높음. **PRD Tier 비용 한도 확정 선행 필수 (미확정 시 Sprint 3 블로커)** |
| Sprint 4 | **Layer 1** | OpenClaw 가상 사무실 | 에셋 선행 필수. **Stage 1 Technical Research 에셋 품질 승인 완료 선행 조건** |

### Core Features (In Scope)

**Layer 0 — UXUI 완전 리셋 (전 Sprint 병행):**
- Subframe 기반 디자인 시스템 + Phase 0 아키타입 테마 선택 (Pre-Sprint 완료)
- 하드코딩 색상 428곳 → `themes.css` 토큰 전환 (ESLint 게이팅)
- Dead button 0개 목표 (Playwright E2E 검증)
- Phase 0 게이팅: Subframe 기준 대비 구현 일치율 ≥ 95%
- **신규 페이지 사이드바 IA 결정**: n8n 관리(Admin) + /office read-only(Admin) + /office(CEO) 배치 위치 → Layer 0에서 선행 결정 (해당 기능 Sprint 착수 전 완료)
- **선행 조건 이유**: CEO `/office` 픽셀 아트 감성과 앱 전체 UI 테마의 시각적 일관성 확보 — Layer 0가 내부 부채 해소임과 동시에 v3 아이덴티티(OpenClaw 감성)의 기반임
...
- n8n Docker 컨테이너 (API-only 모드, 포트 5678 내부망 전용)
- Hono 리버스 프록시: `/admin/n8n/*` → `localhost:5678` (기존 JWT 인증 재활용)
...
| 기존 485 API 변경 | Zero Regression 절대 규칙 |
...
| PixiJS 에셋 외부 구매 | AI 직접 생성으로 대체 (Stage 1 Technical Research 포함) |
...
3. **n8n 보안**: 포트 5678 외부 차단 확인 + Hono 프록시 인증 통과
...
7. **Reflection 비용 한도**: Tier별 한도 PRD 확정 후 구현 (미확정 시 Sprint 3 블로커)
8. **에셋 품질 승인**: Stage 1 Technical Research 에셋 방향 승인 완료 (Sprint 4 착수 선행 조건)

### Future Vision (v4+)

- 멀티모달 에이전트 (음성, 이미지 처리)
- OpenClaw 멀티플레이어 (여러 CEO가 같은 가상 사무실 접속)
