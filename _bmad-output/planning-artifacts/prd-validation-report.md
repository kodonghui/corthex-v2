...
validationStepsCompleted: [step-v-01-discovery, step-v-02-format-detection, step-v-03-density-validation, step-v-04-brief-coverage, step-v-05-measurability, step-v-06-traceability, step-v-07-implementation-leakage, step-v-08-domain-compliance]
...
**Impact:** Moderate — tool responses from external APIs (n8n webhooks, KIS API, web scraping) could contain adversarial content.
...
**Impact:** Low-Moderate — without these fields, Reflection quality may be lower, and Planning search may return less relevant memories.
**Recommendation:** Add `confidence FLOAT DEFAULT 1.0` and `domain_type VARCHAR(50)` to observations table schema. Or document as Sprint 3 architectural decision (deferred optimization).
...
**Impact:** Moderate — without a capability gate, Sprint 3 could "pass" with working infrastructure but zero actual improvement.
**Recommendation:** Add Go/No-Go #9: "Capability Evaluation — 동일 태스크 3회 반복 시 3회차 재수정 횟수가 1회차 대비 ≤ 50%. Memory infrastructure works AND improves outcomes."
...
**Recommendation:** Add NFR-LOG4: "핸드오프 체인 감사 — 모든 call_agent 핸드오프 이벤트 100% activity_logs 기록, 체인 복원 가능" and NFR-LOG5: "메모리 수정 감사 — Reflection 생성/삭제/Planning 적용 100% activity_logs 기록"
