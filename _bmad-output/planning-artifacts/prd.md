...
terminology:
  ...
  에이전트메모리: "3단계 아키텍처: 관찰(observations 신규 테이블) → 반성(reflections 신규 테이블, memory-reflection.ts 크론) → 계획(reflections pgvector 검색 → Soul 주입). 기존 agent_memories 유지(Zero Regression), 신규 2테이블 추가"
...
- Option B 채택 — 기존 agent_memories 유지 + observations/reflections 신규 2테이블 추가, 대체 아님 (Zero Regression)
...
| 6 | Stitch 2 디자인 토큰 추출 완료 | 1 | DESIGN.md + tokens.css 생성 확인 |
...
| 9 | **Capability Evaluation** | 3 | 동일 태스크 N≥3회 반복 시 3회차 재수정 ≤ 1회차의 50%. 표준화된 task corpus 3개, 재수정=에이전트 도구 재호출 수. Memory 인프라 동작 + 결과 개선 모두 검증 |
...
- FR-MEM4: [Sprint 3] 반성 워커가 최근 관찰 20개를 Haiku API로 요약하여 `reflections` 테이블에 고수준 인사이트를 저장한다 (비용: NFR-COST3 Haiku ≤ $0.10/일)
...
- FR-TOOLSANITIZE3: [Sprint 2] 도구 응답 보안 검증이 10종 adversarial payload에 대해 100% 차단율을 달성한다 (PER-1 personality sanitization과 별도 — 도구 응답 경유 공격 표면)
