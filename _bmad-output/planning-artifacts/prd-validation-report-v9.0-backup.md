...
- Executive Summary: ✅ Present (L273)
- Success Criteria: ✅ Present (L471)
- Product Scope: ✅ Present (L668)
- User Journeys: ✅ Present (L1070)
- Functional Requirements: ✅ Present (L2285)
- Non-Functional Requirements: ✅ Present (L2499)
...
- Domain-Specific Requirements (SEC, SDK, DB, ORC, SOUL, OPS, NLM, VEC, N8N-SEC, PER, MEM, PIX, MKT, NRT — 80 domain requirements)
...
**Recommendations:**
- Line numbers revalidated after pre-sweep fixes shifted PRD content (+8 to +156 lines). All L2 header references updated to current PRD.
- Domain requirement count corrected: 75 → 80 (N8N-SEC 6→8, MEM 5→7, NRT 4→5).
...
- PRD L295: "비전: 'AI 조직이 살아 숨 쉰다 — 보이고, 생각하고, 성장한다.'" — equivalent phrasing, fully aligned

**Target Users:** Fully Covered
- Brief §2: Admin 이수진 (32세, AI 시스템 운영), CEO 김도현 (38세, SaaS 대표), Secondary (일반 직원)
- PRD L332-358: All three personas preserved with matching attributes + v2 personas (CEO 김대표, 팀장 박과장, 투자자 이사장) retained

**Problem Statement:** Fully Covered
- Brief §1: 3 problems (블랙박스, 획일성, 정체) + 2 additional (자체 워크플로우 코드, UXUI 428 color-mix)
- PRD L295-322: All 3+2 problems reflected in "What Makes This Special" section (L308) + Executive Summary

**Key Features (4 Layers):** Fully Covered
- Brief Layer 0 (UXUI 리셋) → PRD FR-UX1~3 + UXUI section (L1033)
- Brief Layer 1 (OpenClaw) → PRD FR-OC1~11 + Feature 5-1
- Brief Layer 2 (n8n) → PRD FR-N8N1~6 + FR-MKT1~7 + Feature 5-2
- Brief Layer 3 (Big Five) → PRD FR-PERS1~9 + Feature 5-3 + PER-1~6
- Brief Layer 4 (Memory) → PRD FR-MEM1~14 + Feature 5-4 + MEM-1~7
...
**Go/No-Go Gates (11→14):** Fully Covered
- Brief §4 gates 1-11 (L457-467) → PRD gates 1-14 (L456-469)
- PRD extends Brief's 11 gates with: #9 Observation Poisoning (4-layer defense), #10 Voyage AI Migration (768d→1024d), #14 Capability Evaluation (동일 태스크 3회 반복 ≤ 50% 재수정). Gate #12 v1 Feature Parity and #13 Usability also present.

**Risk Registry:** Fully Covered
- Brief §1 known risks (5 items: PixiJS, n8n, pgvector, UXUI 428, prd.md) → PRD R1-R15 (L396-418) + critics feedback risks
- PRD significantly expands with severity levels, Sprint alignment, mitigation strategies, and fallback plans (15 risks total)

**AHA Moments:** Fully Covered
- Brief §2 Admin AHA: "이게 진짜 개성이네" → PRD Journey 4 (L1152)
- Brief §2 CEO AHA: "/office WOW" → PRD Journey 9 (L1237)
- Brief §2 CEO AHA: "이 에이전트가 성장했다" → PRD Journey 1 (L1074)

**Differentiators:** Fully Covered
- Brief §1: 5 differentiators → PRD L308-322: 6 differentiators (adds "자동화 — n8n")

**Competitive Analysis:** Fully Covered
- Brief §1: OpenAI Assistants, AutoGen/CrewAI, AI Town, n8n 단독 → PRD Innovation section (L1538+): detailed comparison tables for each innovation

**Out of Scope:** Fully Covered
- Brief §4 Out of Scope (9 items) → PRD Out of Scope (L1007-1020): all items preserved + additional code boundary definitions

**Onboarding Flow:** Fully Covered
- Brief §2: Admin-first mandatory sequence → PRD Journey 7 (L1197) + FR59-61
...
- Brief §4: "costs 전면 제거" (CLI Max 월정액) → PRD L253-259, NFR-S7 삭제, NFR-D7 삭제
- Brief §4: "workflows → n8n" → PRD FR-N8N3

**Success Metrics (Brief §3):** Fully Covered
- Brief §3 (L320-370): 4 User Success metrics, 5 Layer-specific KPI sets (~15 metrics), 4 Business Objectives
- PRD Success Criteria (L471-550): 18+ metrics covering all Brief §3 areas. v2 10 criteria + v3 8 criteria with measurement methods.

**Technical Constraints (Brief §5):** Fully Covered
- Brief §5 (L483-495): VPS specs, WebSocket 16→17, Docker 2G, Neon PostgreSQL, CLI Max
- PRD Technical Architecture Context (L1784+): comprehensive coverage of all constraints

**Future Vision v4+ (Brief §4):** Fully Covered
- Brief §4 (L469-479): 9 v4+ items (멀티모달, 멀티플레이어, Redis, 외부 고객 대면, WCAG AA, 글로벌 인사이트, 트리아지 강화, 선호도 학습)
- PRD Out of Scope (L1007-1020): all deferred items documented with phase references

#### Coverage Summary

**Overall Coverage:** 100% — Complete coverage of all 17 Product Brief areas

**Critical Gaps:** 0
**Moderate Gaps:** 0
**Informational Gaps:** 0

~~Previously reported WebSocket channel discrepancy was incorrect — Brief consistently says "16→17채널" (L125, L176, L433, L494), matching PRD exactly. No actual discrepancy exists.~~

**Recommendation:**
PRD provides complete coverage of the Product Brief across all 17 checked areas. No gaps found. Go/No-Go gates expanded from Brief's 11 to PRD's 14 (adding Observation Poisoning, Voyage AI Migration, Capability Evaluation). FR counts expanded (FR-PERS1~9, FR-MEM1~14, MEM-1~7) beyond Brief minimums.
...
#### MISSED-1 (HIGH): Reflections Table Architecture Contradiction — **RESOLVED (v9.2 Pre-sweep 2026-03-22)**
**Identified by:** quinn (confirmed by john)

~~The PRD contradicts itself on a fundamental Sprint 3 architecture decision.~~
**Fixed:** PRD 전면 수정 완료. Option B 일관 적용:
- L148: "reflections는 별도 테이블 아닌 agent_memories 확장 (Zero Regression)"
- L892: "별도 reflections 테이블 아님"
- L1479 MEM-1: "별도 reflections 테이블 생성 안 함"
- L2473 FR-MEM4: "agent_memories 테이블에 memoryType='reflection'으로 저장 (Option B — 별도 reflections 테이블 없음)"
- Feature 5-4, FR-MEM5, FR-MEM8, Migration list, Scope — 모두 agent_memories 확장으로 통일됨.

#### MISSED-2 (MEDIUM): FR-MEM4 Gemini vs Haiku API Contradiction — **RESOLVED (v9.2 Pre-sweep 2026-03-22)**
**Identified by:** quinn

~~FR-MEM4 says "Gemini API로 요약" but all other references use Haiku.~~
**Fixed:** PRD L2473 FR-MEM4 → "Haiku API로 요약" 수정 완료. NFR-COST3 ($0.10/day) Haiku 기준과 일치. Gemini 금지 정책 반영됨.
...
- FR-PERS1 through FR-PERS9 = 9
- FR-MEM1 through FR-MEM14 = 14
- FR-TOOLSANITIZE1 through FR-TOOLSANITIZE3 = 3
- FR-UX1 through FR-UX3 = 3

**Format Violations ("[Actor] can [capability]" / "~할 수 있다"):** 10

| # | FR | Line | Issue |
|---|-----|------|-------|
| 1 | FR-PERS4 | L2461 | "성격 변경이 다음 세션부터 즉시 반영된다" -- passive voice, no actor. Fix: "Admin이 성격을 변경하면 해당 에이전트의 다음 세션부터 즉시 반영된다" |
| 2 | FR-PERS5 | L2462 | "코드 분기(if/switch) 없이 프롬프트 주입만으로 구현된다" -- implementation constraint, not a user capability. Should be NFR or domain constraint |
| 3 | FR-N8N3 | L2439 | "기존 워크플로우 자체 구현 코드(서버 라우트 + 프론트 페이지)가 삭제된다" -- no actor, describes engineering action not user capability |
| 4 | FR-MKT4 | L2451 | "AI 도구 엔진 설정 변경이 다음 워크플로우 실행부터 즉시 반영된다" -- passive voice, no actor |
| 5 | FR-MKT5 | L2452 | "온보딩 시 '마케팅 자동화 템플릿 설치할까요?' 제안이 표시된다" -- passive voice, no actor |
| 6 | FR-PERS9 | L2466 | "성격 슬라이더가 키보드 ←→로 조작 가능하며..." -- passive voice, no actor. Fix: "User가 성격 슬라이더를 키보드 ←→로 조작할 수 있다" |
| 7 | FR-MEM12 | L2481 | "observation 저장 전 4-layer 콘텐츠 방어가 적용된다" -- passive voice, no actor. Describes system behavior not user capability |
| 8 | FR-MEM14 | L2483 | "Reflection 크론 비용이 일일 한도를 초과하면 크론이 자동 일시 중지되고 Admin에게 알림이 전송된다" -- passive voice for auto-pause. Mixed: second clause "Admin 확인 후 재개" has implicit actor |
| 9 | FR-TOOLSANITIZE2 | L2488 | "프롬프트 주입이 감지된 도구 응답은 '[BLOCKED: suspected injection]'으로 교체되고 감사 로그에 기록된다" -- passive voice, no actor. System security action |
| 10 | FR-TOOLSANITIZE3 | L2489 | "도구 응답 보안 검증이 10종 adversarial payload에 대해 100% 차단율을 달성한다" -- passive voice. Describes test/validation outcome, not user capability. Should be NFR or Go/No-Go gate criterion |
...
**Total NFRs Analyzed:** 76

**Counting Breakdown:**
- NFR-P1 through P17 = 17 (Performance)
- NFR-S1 through S10 (minus S7 deleted) = 9 (Security)
...
- NFR-O1 through O11 = 11 (Operations)
...
1. L2294 FR-N8N4: `Docker compose: memory: 2G, cpus: '2'` references and Docker-specific configuration. While "Docker container" is an acceptable scope constraint (n8n is deployed as Docker), the specific compose syntax (`memory: 2G, cpus: '2'`) is implementation detail. FR should reference NFR-SC9 for resource limits instead.
...
4. L2471 FR-MEM2: `embedding VECTOR(1024)` — specific column type/dimension.
...
2. L1459 N8N-SEC-5: `memory: 2G, cpus: '2'. OOM 시 자동 재시작 (restart: unless-stopped)` — Docker Compose syntax.
...
| N8N-SEC | 8 | Complete — port blocking, Admin-only, tag isolation, HMAC, Docker limits, DB access, resource cap, webhook verification |
| PER (Personality) | 6 | Complete — sanitization, extraVars, defaults, fallback, accessibility, tooltips |
| MEM (Memory) | 7 | Complete — zero regression, tier limits, isolation, permissions, audit, observation defense, reflection integrity |
...
| NRT (NEXUS Real-time) | 5 | Complete — state model, heartbeat, WebSocket broadcast, sync delay, graceful degradation |

**Total: 80/80 domain requirements present and documented.**
...
| **subscription_tiers** | Present | L1745-1772 | CLI Max model documented. v3 cost additions table (n8n Docker, Reflection LLM, external APIs, Voyage AI Embedding). Phase 5+ billing models explored (4 candidates) |
| **integration_list** | Present | L1774-1801 | v2: 9 integrations (SDK, Telegram, KIS, NotebookLM, SketchVibe, Voyage AI, ARGOS, PostgreSQL, WebSocket). v3: 7 new (n8n Docker, n8n editor, /ws/office, Memory Reflection, soul-enricher, AI tool engines, SNS) |
...
| FR-MEM4 | 4 | 4 | 4 | 5 | 5 | 4.4 | — | ~~API contradiction~~ **RESOLVED** (v9.2 Pre-sweep): now says "Haiku API". Consistent with NFR-COST3 |
...
- ~~**reflections table architecture contradiction** (MISSED-1)~~ — **RESOLVED** (v9.2 Pre-sweep 2026-03-22). PRD 전면 수정: Option B 일관 적용 (agent_memories 확장, 별도 reflections 테이블 없음).
- ~~**API model contradiction** (MISSED-2)~~ — **RESOLVED** (v9.2 Pre-sweep 2026-03-22). FR-MEM4 "Haiku API로 요약"으로 수정 완료. Gemini 금지 정책 반영.
- ~~**Stale references**~~ — **RESOLVED** (v9.2 Pre-sweep 2026-03-22). L102 → "Subframe 폐기 → Stitch 2 전환" 명시. Go/No-Go #6 → Stitch 2 수정. L167 → "Stitch 2 DESIGN.md 기반" 수정.
...
- Developer clarity: **Strong** — 116 FRs with phase/sprint tags, 80 domain requirements, code impact estimates with line counts
...
| Domain Awareness | Met | V-08: 80/80 domain requirements across 14 categories. All 4 domain areas covered |
...
**ISSUE-1 (HIGH): Reflections Table Architecture Contradiction — RESOLVED (v9.2 Pre-sweep 2026-03-22)**
~~PRD contradicted itself on reflections storage.~~
**Fixed:** PRD 전면 수정 — Option B 일관 적용. L148, L892, L1479, L2473 모두 "agent_memories 확장, 별도 reflections 테이블 없음".

**ISSUE-2 (MEDIUM): FR-MEM4 Gemini vs Haiku API — RESOLVED (v9.2 Pre-sweep 2026-03-22)**
~~FR-MEM4: "Gemini API로 요약"~~
**Fixed:** PRD L2473 FR-MEM4 → "Haiku API로 요약". NFR-COST3 ($0.10/day) Haiku 기준 일치.

#### Overall Quality Rating

**Rating:** 4/5 — Good

This is a strong, well-structured PRD with exceptional v2 foundations and comprehensive v3 additions. The 2,491-line document maintains consistent quality across 11 sections, 116 FRs, 74 NFRs, and 80 domain requirements. The two internal contradictions (reflections table, Gemini vs Haiku) have been **RESOLVED** in v9.2 Pre-sweep. Remaining issue: scattered deferred items (CR-2) prevent an "Excellent" rating.

#### Top 3 Improvements

1. ~~**Resolve reflections table architecture contradiction (ISSUE-1)**~~ — **RESOLVED** (v9.2 Pre-sweep)

2. ~~**Align FR-MEM4 API + update stale references (ISSUE-2 + CR-3)**~~ — **RESOLVED** (v9.2 Pre-sweep)
...
**To make it great:** ~~Resolve the reflections table contradiction~~ ✅, ~~align the Gemini/Haiku reference~~ ✅ — both resolved. Remaining: move implementation details from v3 FRs to architecture, and consolidate scattered deferred items.
...
| V-08 | Domain Compliance | PASS | — | 80/80 domain requirements. 4 domain areas fully covered |
...
| MISSED-1: reflections table contradiction | ~~HIGH~~ | **RESOLVED** (v9.2 Pre-sweep 2026-03-22) |
| MISSED-2: FR-MEM4 Gemini vs Haiku | ~~MEDIUM~~ | **RESOLVED** (v9.2 Pre-sweep 2026-03-22) |
...
3. ~~Resolve reflections table contradiction~~ — **RESOLVED** (v9.2 Pre-sweep)
4. ~~Align FR-MEM4 API reference (Gemini → Haiku)~~ — **RESOLVED** (v9.2 Pre-sweep)
