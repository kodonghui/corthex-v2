...
If you see `#faf8f5`, `olive`, `cream`, `cyan-400`, `slate-950` or ANY specific color in this file → it is a BUG.
The previous pipeline (v5.1) failed because hardcoded colors from old themes leaked into every execution.
...
- **KDH Party Mode**: 3-critic review (from `/kdh-full-auto-pipeline` v9.2). TeamCreate mandatory. Score >= 7/10.
...
- `resume` or `계속`: Read pipeline-status.yaml → find first non-complete phase → resume
  - `계속` = run to completion. Do NOT stop at intermediate milestones.
...
Default 3 themes (user can customize):
```yaml
themes:
  - name: "command"
...
Identical to v5.1. Runs ONCE. Detects framework, router, all page routes, tailwind config, app shell files.

```
Step 0: Project Auto-Scan
  1. Read package.json → detect framework (React/Vue/Svelte/Next/Nuxt/Angular)
  2. Find tsconfig.json → determine type-check command
  3. Find router file → extract ALL page routes (including admin routes if separate package)
  4. Find tailwind.config → extract current design tokens
  5. Find entry file → detect dark/light mode setting
  6. Find layout/sidebar files → identify app shell
  7. Find index.html → check font CDN links
  8. Find architecture/PRD docs
...
```yaml
packages:
  app:      # CEO app — all user-facing pages
...
| 2 | Design System | **3R party × N themes** | 2 | **promax --design-system** |
...
**This is the #1 reason v5.1 failed. Text-only research produces text-only designs.**
...
FOR EACH screenshot:
  ...
  3. Rate relevance to CORTHEX (1-5)
...
FOR EACH theme in themes list:
  1. Run promax --design-system with theme keywords:
     python3 .claude/skills/ui-ux-pro-max/scripts/search.py "{keywords}" \
       --design-system --persist -p "CORTHEX-{theme-name}"
  ...
  3. Read promax output → enhance with Phase 1 visual research patterns:
     - "8/15 benchmarks used collapsible sidebar → adopt this"
     - "Top 3 sites used this spacing rhythm → apply to our tokens"

  4. Validate with 60-30-10 rule (from Brand Systems):
     - 60% dominant (backgrounds/surfaces): verify promax bg colors
     - 30% secondary (supporting): verify promax secondary
     - 10% accent (CTAs/highlights): verify promax CTA color
  ...
  6. Generate page-specific overrides for key pages:
     python3 search.py "{keywords}" --design-system --persist \
       -p "CORTHEX-{theme-name}" --page "dashboard"
     (repeat for: hub, landing, settings, agents)
...
```
FOR EACH theme:
  1. Call Stitch MCP create_project: "CORTHEX-{theme-name}"
...
```
FOR EACH theme:
  FOR EACH package (app, admin, landing):
...
FOR EACH package:
  1. Generate color-mapping.md:
     ...
     (e.g., bg-blue-500 from Stitch → var(--color-primary))
...
2. Build landing page in packages/landing/:
...
4. Connect routing: / → landing, /app → CEO app, /admin → admin
5. Type-check
...
5. **ALL packages covered.** app + admin + landing. No package left behind.
...
8. **`계속` = run to completion.** Not done until Phase 5 complete + type-check + deployed.
