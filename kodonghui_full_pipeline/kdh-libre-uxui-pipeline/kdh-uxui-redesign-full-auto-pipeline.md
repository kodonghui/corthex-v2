...
description: 'UXUI Redesign Pipeline v5.1 (Universal). Stitch 2 native — React/JSX direct export + DESIGN.md. 8 Phases + auto-scan + app shell sync + completeness gates + E2E verification. Libre+BMAD+KDH.'
---

# Universal UXUI Redesign Pipeline v5.1

Phase 0~7 fully automated. **Stitch 2 MCP** for UI generation (Phase 6) with React/JSX direct export + DESIGN.md design systems.
Works on ANY frontend project — auto-detects framework, router, design tokens, and app shell.
Output root: `_uxui_redesign/`

**Stitch 2 (2026-03-19):** Infinite AI canvas, voice interaction, 5-screen simultaneous generation, DESIGN.md agent-friendly design systems, React/JSX + Tailwind CSS direct export, official SDK + MCP server.
...
- `resume` or `계속`: Read pipeline-status.yaml → find first non-complete phase → resume from there
  ...
  - After Stitch React/JSX generation (Phase 6), IMMEDIATELY proceed to framework conversion (Phase 7).
...
## Phase 5: DESIGN.md + Stitch 2 Prompts

**Libre Tools:** Premium SaaS Design (prompt patterns)

### Step 5-1: Generate DESIGN.md (3R)

**Output:** `phase-5-prompts/DESIGN.md`

**Writer Instruction:**
```
[Step Instruction] Create Stitch 2 DESIGN.md — agent-friendly design system file.
Output: _uxui_redesign/phase-5-prompts/DESIGN.md

FIRST: Read _uxui_redesign/project-context.yaml for all page routes.

BEFORE WRITING — Read:
- .claude/plugins/design-mastery/skills/premium-saas-design/SKILL.md
- ALL previous outputs (winning theme + tokens + components from Phase 0-4)

DESIGN.md is Stitch 2's native design system format. It tells Stitch HOW to generate screens.
Create a complete DESIGN.md including:

1. Brand Identity — project name, archetype personality, emotional tone
2. Color Palette — exact hex values for: primary, secondary, accent, background, surface, border, text hierarchy
3. Typography — font families (heading/body/mono), sizes, weights, line-heights
4. Spacing Scale — base unit, multipliers, padding/margin/gap values
5. Component Rules — card radius, shadow, button styles, input styles, table styles
6. Layout Rules — sidebar width, grid patterns, responsive breakpoints
7. Icon Library — which icon set (e.g., Lucide React), style consistency rules
8. Dark/Light Mode — which mode, how to handle (forced vs auto)
9. Animation — transition durations, easing functions, hover effects
10. Do/Don't — explicit rules (e.g., "DO use olive #283618 for sidebar, DON'T use Material Symbols")
11. **App Shell Rule (CRITICAL)** — "Each page generates CONTENT AREA ONLY. DO NOT generate sidebar, topbar, or navigation. The app has a shared Layout component with a fixed sidebar. If you generate a sidebar inside a page, you get sidebar-inside-sidebar. NEVER do this."
12. **No Hardcoded Colors** — "ALL colors must use Tailwind utility classes from the design tokens above. NEVER use inline hex values like bg-[#283618]. Use the semantic token names (e.g., bg-sidebar, text-primary)."

Format: Markdown sections with exact values. NO vague descriptions. NO "[fill in]" placeholders.
This file will be uploaded to Stitch 2 to ensure ALL generated screens follow the same design system.
```

### Step 5-2: Web Stitch Prompts (3R)

**Output:** `phase-5-prompts/stitch-prompt-web.md`

**Writer Instruction:**
```
[Step Instruction] Create per-page Stitch 2 generation prompts for web.
Output: _uxui_redesign/phase-5-prompts/stitch-prompt-web.md

FIRST: Read _uxui_redesign/project-context.yaml for all page routes.
Read DESIGN.md from phase-5-prompts/ for design system context.

For EACH page route in project-context.yaml:
1. Page name and purpose
2. Layout structure (sidebar+content, full-width, etc.)
3. Key sections and their content
4. Data tables, charts, cards needed
5. Interactive elements (buttons, forms, dropdowns)
6. "Reference DESIGN.md for all colors, fonts, spacing"
7. "Generate as React/JSX with Tailwind CSS utility classes" (Stitch 2 native React export)

Stitch 2 generates up to 5 screens at once — group pages into batches of 5.
Include archetype+card combination as context in every prompt section.
NO placeholders. COPY-PASTE READY for Stitch 2 MCP.
```

### Step 5-3: App Stitch Prompts (3R)

**Output:** `phase-5-prompts/stitch-prompt-app.md`
Same structure for mobile app with responsive/mobile-specific instructions. Include archetype+card context.

---

## Phase 6: Stitch 2 Generation (Automated via MCP)

**Requires:** Stitch MCP server configured in MCP settings
**Stitch 2 Key Change:** Generates React/JSX + Tailwind CSS directly (no HTML→React conversion needed in Phase 7).

### Step 6-1: Create Stitch Project + Upload DESIGN.md

**Orchestrator Action (no team needed):**
```
1. Read project-context.yaml for project name
2. Call MCP tool `create_project` with name "{project}-v3-redesign"
3. Save returned projectId to pipeline-status.yaml under phase-6.projectId
4. Upload DESIGN.md: Read phase-5-prompts/DESIGN.md → include as design system context in all subsequent generate calls
```

### Step 6-2: Generate Web Screens (5 at a time)

**Orchestrator Action:**
```
1. Read phase-5-prompts/stitch-prompt-web.md
2. Read project-context.yaml → get FULL page list
3. Group pages into batches of 5 (Stitch 2 generates up to 5 simultaneously)
4. For EACH batch:
   a. Call MCP tool `generate_screen_from_text` with:
      - projectId from step 6-1
      - prompt text (from Phase 5) + DESIGN.md reference
      - Request React/JSX + Tailwind CSS output format
   b. Call `get_screen` → save React/JSX code to phase-6-generated/web/{screen-name}.tsx
   c. Save screenshot to phase-6-generated/web/{screen-name}.png (if available)
5. For pages NOT in Phase 5 prompts:
   → Auto-generate prompt from existing page code + DESIGN.md
   → Generate via MCP
6. Verify: ALL routes from project-context.yaml have corresponding .tsx files
7. git commit "docs(uxui-redesign): Phase 6-2 web screens generated via Stitch 2 MCP (React/JSX)"
```

**Expected: ALL page routes must have a corresponding Stitch-generated React component.**
...
Save to phase-6-generated/app/{screen-name}.tsx and .png
git commit "docs(uxui-redesign): Phase 6-3 app screens generated via Stitch 2 MCP"
```
...
**Orchestrator Action:**
```
Read phase-5-prompts/ for landing prompt (if exists, else derive from Phase 2-3 + DESIGN.md)
Generate via Stitch 2 MCP (React/JSX) → save to phase-6-generated/landing/
git commit "docs(uxui-redesign): Phase 6-4 landing page generated via Stitch 2 MCP"
```

### Step 6-5: Visual + Code Review (1R)

**Orchestrator Action:**
```
1. Read ALL generated screenshots (.png) from phase-6-generated/
2. Read ALL generated React/JSX (.tsx) from phase-6-generated/
3. Visual check: color accuracy, typography, layout vs DESIGN.md
4. Code check: imports valid, no hardcoded colors (should use Tailwind tokens), no Subframe references
5. If major discrepancy:
   → Re-generate with refined prompt + "IMPORTANT: follow DESIGN.md exactly"
   → Max 2 retries per screen
6. Write review to phase-6-generated/stitch-review.md
7. git commit "docs(uxui-redesign): Phase 6 complete — Stitch 2 generation reviewed"
```

### Stitch 2 Advantages Over v1

- React/JSX direct export → Phase 7 page rebuild is MUCH simpler (adapt, not rewrite from HTML)
- DESIGN.md → consistent design system across all generated screens
- 5-screen batches → faster generation
- Voice interaction → can refine screens conversationally
...
3. Write instructions: "Upload DESIGN.md + paste prompts from phase-5-prompts/ into https://stitch.withgoogle.com/"
...
**Output:** Actual working framework pages that LOOK LIKE the Stitch React/JSX designs.

**CRITICAL RULE: This is a REBUILD, not a patch.**
- DO NOT "add classes to existing pages"
- DO rewrite each page's render/return output to match Stitch React/JSX structure
- DO preserve existing hooks, API calls, state management, event handlers
- The page must VISUALLY MATCH the Stitch React/JSX when rendered
...
2. Generate dynamic color mapping (Step 7-1 color mapping):
   ...
   b. Read Stitch React/JSX inline config (from any phase-6-generated HTML file)
...
3. For each page with a Stitch React/JSX counterpart:
   a. Read the Stitch React/JSX file (phase-6-generated/web/{screen}.html)
   b. Read the existing page file (from project-context.yaml pages list)
   c. EXTRACT from Stitch React/JSX:
   ...
   e. REWRITE the render/return output:
      - Structure matches Stitch React/JSX layout
      - Utility classes match Stitch React/JSX (standardized via color-mapping.md)
      - Dynamic data replaces Stitch's hardcoded demo data
      - Responsive: mobile-first
   f. Verify: type-check passes

4. For pages WITHOUT Stitch React/JSX:
...
5. App Shell was already synced in Step 7-0 — do NOT re-modify layout/sidebar here.
   - If Stitch React/JSX pages have their own sidebar/nav, IGNORE them
   - Use ONLY the main content area from each Stitch React/JSX
```
...
- The exact Stitch React/JSX file path to read
...
Step 7-1.5: File Completeness Gate
  ...
  4. If file is MISSING:
     a. Check _uxui_redesign/phase-6-generated/ for a corresponding Stitch React/JSX
     b. If found → create the page file from Stitch React/JSX + design tokens
...
Step 7-2: Live Render Verification (MANDATORY)
  ...
  Option A (Playwright / browser automation available):
    ...
    5. Compare screenshots against reference:
       ...
       - If no Stitch PNG → verify structural match against Stitch React/JSX
  ...
  Option B (Playwright NOT available):
    1. For EACH page file:
       ...
       b. Read corresponding Stitch React/JSX (if exists)
...
PHASE 7 (Sequential + Parallel Execution):
  ...
  Step 7-1: Page Rebuild (parallel agents)
    ...
    → Each agent: read Stitch React/JSX + read existing page → rebuild render output → commit
...
1. **"Add classes" instead of "rebuild render output"** — Agent adds `sm:p-4` to existing page instead of rewriting render output to match Stitch React/JSX. RESULT: Page looks nothing like the design. FIX: Phase 7 instruction explicitly says "REBUILD the render output" and "the page must VISUALLY MATCH the Stitch design." Verify by comparing output structure against HTML.
2. **Ignoring Stitch React/JSX structure** — Agent reads Stitch React/JSX but only extracts colors, ignores layout/sections/ordering. RESULT: Colors match but layout is completely different. FIX: Extract FULL structure (section ordering, grid patterns, card hierarchy) not just tokens.
...
4. **Treating Stitch nav/sidebar as authoritative** — Each Stitch React/JSX has its own sidebar/nav that differs between screens. FIX: IGNORE Stitch's nav. Use only the main content area. App shell (layout + sidebar) is the single source of truth for navigation.
...
12. **(v5.1) "Old UI library leaking into new design"** — After redesign, old component imports (e.g., `from "ui/components"`, Subframe-generated components) remain in pages. RESULT: mixed design systems, inconsistent styling, v1→v2 hardcoded-color incident repeat. FIX: Phase 7 completion gate runs `grep -r "subframe\|from.*ui/components"` — ANY match = FAIL. All old UI component imports must be replaced with Stitch 2 generated components.
...
[ ] Phase 7-1: pages VISUALLY MATCH Stitch React/JSX (not just "classes added")
[ ] Phase 7-1: verify by comparing rebuilt output vs Stitch React/JSX — same structure, same colors, same layout
...
13. **Phase 7 = REBUILD, not patch.** Each page's render output must be REWRITTEN to match Stitch React/JSX structure. "Adding responsive classes to existing pages" is NOT Phase 7 completion. The page must look like the Stitch design when rendered.
...
15. **Stitch React/JSX = visual spec, not drop-in code.** Extract layout/colors/typography from Stitch. Replace hardcoded demo data with real hooks. Map Stitch's inline config to design-tokens.md values via color-mapping.md. Map Stitch icons → project's icon library.
16. **Stitch inconsistencies (sidebar/nav) = ignore.** Each Stitch React/JSX is standalone — they have different sidebars, navs, etc. Use ONLY the main content area from each Stitch React/JSX. The app shell (layout + sidebar) is shared and consistent, synced in Step 7-0.
17. **Visual verification before completion.** After rebuilding each page, compare the output structure against Stitch React/JSX. Same section ordering, same card patterns, same color usage. If they don't match, it's not done.
