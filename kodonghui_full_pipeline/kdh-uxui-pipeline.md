...
```
전문가별 적대적 모드 행동:
- John(PM): "이 기능 기존에 됐는데 왜 빠졌죠?" — FEATURE_SPEC 하나하나 대조
...
### John (PM)
{적대적으로 FEATURE_SPEC 대조, 누락 기능 지적}
...
- [x/] data-testid가 모든 인터랙션 요소에 할당됨 — {N}개
- [x/] 기존 기능 전부 커버 — {FEATURE_SPEC 참조 결과}
...
```
[파일럿] 1번 페이지를 spec → image → code 끝까지 완주
  ↓ 파이프라인 검증 완료
...
2. v1 테마에 얽매이지 않음 — 모던 SaaS 생산성 도구 방향
...
```
인자:
- spec-batch 1: 1순위 페이지들 (이미 완료된 건 건너뜀)
- spec-batch 2: 2순위 페이지들
- spec-batch 3: 3순위 페이지들

실행 순서:
1. PROGRESS.md에서 해당 우선순위의 미완료 페이지 목록 확인
2. 각 페이지에 대해 순차적으로:
   a. 페이지 코드 읽기
   b. FEATURE_SPEC에서 기능 확인 (있으면)
...
```
1. "콘텐츠 영역만 디자인" — 앱에 이미 사이드바/헤더가 있으므로 포함 금지
2. 기존 테마에 얽매이지 않음 — 모던 SaaS 생산성 도구 방향
3. "톤/레이아웃 자유" — Gemini가 최적 판단
4. "설명서 파일 읽기" — specs/{번호}-{페이지명}.md를 먼저 읽고 기능 요소 파악
5. "실제 앱 스크린샷처럼" — 와이어프레임/목업 아닌 프로덕션 품질
6. 저장 위치: _uxui-refactoring/designs/{번호}-{페이지명}-desktop.png, mobile.png
