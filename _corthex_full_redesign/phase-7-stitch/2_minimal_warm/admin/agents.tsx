/**
 * Agent Management — Minimal Warm Theme
 *
 * API Endpoints:
...
const inputCls = 'w-full border-[#e8e4d9] rounded-lg focus:ring-[#81b29a] focus:border-[#81b29a] py-2 text-sm'
...
export function AgentsPage() {
  ...
  if (!selectedCompanyId) {
    return (
      <div className="flex items-center justify-center h-64" style={{ backgroundColor: '#fcfbf9', color: '#2c2c2c' }}>
  ...
  return (
    <div data-testid="agents-page" className="h-screen overflow-hidden flex" style={{ backgroundColor: '#fcfbf9', fontFamily: "'Inter', sans-serif", color: '#2c2c2c' }}>
      {/* BEGIN: Main Content Area */}
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-white">
        {/* BEGIN: Header */}
        <header className="h-16 border-b flex items-center justify-between px-8 bg-white/80 backdrop-blur-sm sticky top-0 z-10" style={{ borderColor: '#e8e4d9' }}>
          <h1 className="text-lg font-bold" style={{ color: '#2c2c2c' }}>Agent Management</h1>
          <div className="flex items-center gap-4">
            <button
              data-testid="agents-create-btn"
              onClick={() => setShowCreate(true)}
              className="px-4 py-2 text-sm font-medium rounded-lg transition-colors border"
              style={{ color: '#2c2c2c', backgroundColor: '#f5f3ec', borderColor: '#e8e4d9' }}
            >
              New Agent Template
            </button>
          </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <section className="w-80 border-r overflow-y-auto" style={{ borderColor: '#e8e4d9', backgroundColor: '#fcfbf9' }} data-testid="agents-table">
            <div className="p-4 space-y-3">
              {/* Search Box */}
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3">
                  <svg className="w-4 h-4" style={{ color: '#81b29a' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                </span>
                <input
                  data-testid="agents-search-input"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full pl-9 pr-4 py-2 text-sm rounded-lg focus:ring-[#81b29a] focus:border-[#81b29a] bg-white"
              ...
              {isLoading ? (
                <div data-testid="agents-loading" className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="bg-white p-4 rounded-xl border animate-pulse" style={{ borderColor: '#e8e4d9' }}>
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-full" style={{ backgroundColor: '#f5f3ec' }} />
                        <div className="flex-1 space-y-2">
                          <div className="h-4 w-24 rounded" style={{ backgroundColor: '#f5f3ec' }} />
                          <div className="h-3 w-32 rounded" style={{ backgroundColor: '#f5f3ec' }} />
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white p-4 rounded-xl shadow-sm cursor-pointer transition-all group ${
                        isSelected
                          ? 'border-2'
                          : 'border hover:border-[#e8e4d9]'
                      }`}
                      style={{
                        borderColor: isSelected ? '#81b29a' : '#e8e4d9',
                      }}
                    >
                      <div className="flex items-start gap-3">
                        <div className="relative">
                          <div
                            ...
                            style={{ borderColor: '#e8e4d9', backgroundColor: '#f5f3ec', color: '#2c2c2c' }}
                          >
                            {a.name.charAt(0)}
                          </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 data-testid={`agents-name-${a.id}`} className="text-sm font-bold truncate" style={{ color: '#2c2c2c' }}>
                            {a.name}
                          ...
                          <p className="text-xs truncate mb-2" style={{ color: '#81b29a' }}>{a.role || 'No role'}</p>
          ...
          {selectedAgent ? (
            <section className="flex-1 flex flex-col min-w-0 overflow-hidden" data-testid="agents-detail-panel">
              {/* Editor Tabs/Header */}
              <div className="flex items-center justify-between px-8 py-4 border-b bg-white" style={{ borderColor: '#f5f3ec' }}>
                <div className="flex items-center gap-6">
                  <button
                    ...
                    style={{
                      borderColor: detailTab === 'soul' ? '#81b29a' : 'transparent',
                      color: detailTab === 'soul' ? '#2c2c2c' : '#81b29a',
                    }}
                  >
                    Soul Markdown
                  </button>
                  <button
                    ...
                    style={{
                      borderColor: detailTab === 'config' ? '#81b29a' : 'transparent',
                      color: detailTab === 'config' ? '#2c2c2c' : '#81b29a',
                    }}
                  >
                    Configuration
                  </button>
                  <button
                    ...
                    style={{
                      borderColor: detailTab === 'memory' ? '#81b29a' : 'transparent',
                      color: detailTab === 'memory' ? '#2c2c2c' : '#81b29a',
                    }}
                  >
                    Memory Snapshots
                  </button>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs italic" style={{ color: '#81b29a' }}>Auto-saved 2m ago</span>
                </div>
              </div>
              ...
              <div className="flex-1 flex overflow-hidden">
                ...
                {detailTab === 'soul' && (
                  <div className="flex-1 flex flex-col p-6 bg-white overflow-hidden">
                    <div className="flex-1 flex flex-col rounded-xl border overflow-hidden shadow-inner" style={{ borderColor: '#e8e4d9', backgroundColor: '#fcfbf9' }}>
                      <div className="flex items-center justify-between px-4 py-2 border-b bg-white/50" style={{ borderColor: '#e8e4d9' }}>
                        ...
                        <div className="text-[10px] font-mono uppercase tracking-widest" style={{ color: '#81b29a' }}>SOUL_FABRIC_CORE</div>
                        <div className={`text-[10px] font-mono ${charCount > 45000 ? 'text-red-500' : ''}`} style={charCount <= 45000 ? { color: '#81b29a' } : {}}>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 bg-white">
                    ...
                    <div className="max-w-lg space-y-8">
                      ...
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold uppercase tracking-widest" style={{ color: '#81b29a' }}>Core Identity</h4>
                        <div>
                          <label className="block text-sm font-semibold mb-1" style={{ color: '#2c2c2c' }}>Agent Name</label>
                          <input type="text" value={editForm.name || ''} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className={inputCls} />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold mb-1" style={{ color: '#2c2c2c' }}>Primary Role</label>
                          <input type="text" value={editForm.role || ''} onChange={(e) => setEditForm({ ...editForm, role: e.target.value })} className={inputCls} />
                        </div>
                      </div>
                      {/* Intelligence Configuration */}
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold uppercase tracking-widest" style={{ color: '#81b29a' }}>Intelligence</h4>
                        <div>
                          <label className="block text-sm font-semibold mb-1" style={{ color: '#2c2c2c' }}>Tier Level</label>
                          <select
                        ...
                        <div>
                          <label className="block text-sm font-semibold mb-1" style={{ color: '#2c2c2c' }}>Foundation Model</label>
                          <select
                      ...
                      {/* Permissions & Tools */}
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold uppercase tracking-widest" style={{ color: '#81b29a' }}>Permissions &amp; Tools</h4>
                        <div className="space-y-2">
                          {(selectedAgent.allowedTools || []).map((tool) => (
                            <label key={tool} className="flex items-center gap-3 group cursor-pointer">
                              <input defaultChecked type="checkbox" className="rounded text-[#81b29a] focus:ring-[#81b29a]" style={{ borderColor: '#e8e4d9' }} />
                              <span className="text-sm font-medium transition-colors group-hover:text-[#2c2c2c]" style={{ color: '#2c2c2c' }}>{tool}</span>
                            </label>
                          ))}
                        </div>
                        <div className="pt-2 border-t" style={{ borderColor: '#e8e4d9' }}>
                          <label className="flex items-center justify-between cursor-pointer">
                            <div>
                              <span className="text-sm font-bold" style={{ color: '#2c2c2c' }}>isSecretary</span>
                              <p className="text-xs" style={{ color: '#81b29a' }}>Auto-organizes workspace</p>
                            </div>
                            <div className="relative inline-flex items-center">
                              ...
                              />
                              <div className="w-11 h-6 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#81b29a]" style={{ backgroundColor: editForm.enableSemanticCache ? '#81b29a' : '#e8e4d9' }} />
                            </div>
                          </label>
                        </div>
                      </div>
                ...
                {detailTab === 'memory' && (
                  <div className="flex-1 overflow-y-auto p-6 bg-white">
                    <div className="text-center py-16" style={{ color: '#81b29a' }}>
            ...
            <section className="flex-1 flex items-center justify-center bg-white">
              <p className="text-sm" style={{ color: '#81b29a' }}>Select an agent from the list</p>
            </section>
          )}
          {/* END: Right Column */}
        </div>
      ...
      {deactivateTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden border" style={{ borderColor: '#e8e4d9' }}>
            <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: '#f5f3ec', backgroundColor: '#fcfbf9' }}>
              <h3 className="text-lg font-bold" style={{ color: '#2c2c2c' }}>에이전트 비활성화</h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  style={{ borderColor: '#e8e4d9', color: '#2c2c2c' }}
                >
                  취소
                </button>
                <button
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden border" style={{ borderColor: '#e8e4d9' }}>
            <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: '#f5f3ec', backgroundColor: '#fcfbf9' }}>
              <h3 className="text-lg font-bold" style={{ color: '#2c2c2c' }}>New Agent Template</h3>
            ...
            <form data-testid="agents-create-form" onSubmit={handleCreate} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: '#2c2c2c' }}>Agent Name *</label>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: '#2c2c2c' }}>Role</label>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: '#2c2c2c' }}>Tier Level</label>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: '#2c2c2c' }}>Foundation Model</label>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: '#2c2c2c' }}>Department</label>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: '#2c2c2c' }}>Soul (Persona)</label>
                {soulTemplates.length > 0 && (
                  <select
                    ...
                    className="w-full mb-2 border-[#e8e4d9] rounded-lg text-xs py-1.5 focus:ring-[#81b29a] focus:border-[#81b29a]"
                  >
              ...
              <div className="pt-4 flex gap-3">
                <button data-testid="agents-create-cancel" type="button" onClick={() => { setShowCreate(false); setForm({ ...defaultCreateForm }) }} className="flex-1 px-4 py-2.5 border rounded-lg text-sm font-medium transition-colors" style={{ borderColor: '#e8e4d9', color: '#2c2c2c' }}>
                  취소
                </button>
                <button data-testid="agents-create-submit" type="submit" disabled={createMutation.isPending} className="flex-1 px-4 py-2.5 text-white rounded-lg text-sm font-medium transition-all disabled:opacity-50" style={{ backgroundColor: '#81b29a' }}>
      ...
      {showCacheDisableModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 border" style={{ borderColor: '#e8e4d9' }}>
            <h3 className="text-lg font-bold mb-3" style={{ color: '#2c2c2c' }}>Semantic Cache 비활성화</h3>
            <p className="text-sm mb-4" style={{ color: '#2c2c2c' }}>캐시를 비활성화하면 응답 속도가 느려질 수 있습니다.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowCacheDisableModal(false)} className="flex-1 px-4 py-2 border rounded-lg text-sm" style={{ borderColor: '#e8e4d9' }}>취소</button>
