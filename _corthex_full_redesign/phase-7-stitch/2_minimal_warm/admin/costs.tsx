/**
 * Cost & Budget Management — Minimal Warm Theme
 *
 * API Endpoints:
    ...
    <section className="grid grid-cols-1 md:grid-cols-3 gap-6" data-testid="costs-summary-cards">
      {cards.map((c) => (
        <div key={c.label} className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100">
          ...
          <h3 className="text-4xl font-bold" style={{ color: c.color, fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>{c.value}</h3>
          <div className="mt-4 flex items-center gap-2 text-sm" style={{ color: c.trendUp ? '#16a34a' : c.trend ? '#ef4444' : '#e57373' }}>
            {c.trend && (
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-stone-50">
          {sorted.map((r) => (
            <tr key={r.departmentId} className="hover:bg-[#fcfbf9]/50 transition-colors">
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {sorted.map(r => (
            <tr key={r.agentId} className="border-b border-stone-50 hover:bg-[#fcfbf9]/50">
              <td className="py-3 font-medium">{r.agentName}</td>
              <td className="py-3 font-mono" style={{ color: '#e57373' }}>${microToUsd(r.totalCostMicro)}</td>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {sorted.map(r => (
            <tr key={`${r.provider}-${r.model}`} className="border-b border-stone-50 hover:bg-[#fcfbf9]/50">
    ...
    <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-stone-100">
      <div className="flex items-center justify-between mb-6">
        <h4 className="font-bold text-lg flex items-center gap-2">
          <svg className="w-5 h-5" style={{ color: '#e57373' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex gap-1" data-testid="costs-axis-tabs">
          {tabs.map((t) => (
            <button
              ...
              style={tab === t.value ? { backgroundColor: '#e57373' } : {}}
            >
              {t.label}
            </button>
          ))}
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 flex flex-col justify-between">
      <div>
        <h4 className="font-bold text-lg mb-4 flex items-center gap-2" data-testid="budget-panel-title">
          <svg className="w-5 h-5" style={{ color: '#f4a261' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <>
            ...
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input
                  className="w-full pl-8 pr-4 py-3 border-stone-200 rounded-xl focus:border-[#e57373] focus:ring-[#e57373] transition-all"
      ...
      <div className="mt-8 pt-6 border-t border-stone-100">
        ...
        <div className="w-full h-2 bg-stone-100 rounded-full overflow-hidden">
          <div className="h-full rounded-full" style={{ width: `${usagePercent}%`, backgroundColor: '#f4a261' }} />
        </div>
      </div>
    </div>
  )
    ...
    <section className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden">
      <div className="p-6 border-b border-stone-100 flex items-center justify-between">
        ...
        <div className="flex gap-2" data-testid="daily-chart-title">
          <button className="flex items-center gap-2 px-4 py-2 text-stone-600 text-sm font-medium rounded-lg hover:bg-stone-100" style={{ backgroundColor: '#fcfbf9' }}>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-stone-50">
              {items.slice(0, 10).map((d) => (
                <tr key={d.date} className="hover:bg-[#fcfbf9]/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right font-bold" style={{ fontFamily: 'serif', color: '#e57373' }}>${microToUsd(d.costMicro)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="p-4 border-t border-stone-100 bg-stone-50/30 flex justify-center">
        <nav className="flex gap-1">
          ...
          <button className="w-8 h-8 rounded flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: '#e57373' }}>1</button>
...
export function CostsPage() {
  ...
  if (!companyId) {
    return (
      <div data-testid="costs-page" className="p-8" style={{ backgroundColor: '#fcfbf9' }}>
        <h1 className="text-xl font-semibold" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>비용 관리</h1>
  ...
  return (
    <div data-testid="costs-page" style={{ backgroundColor: '#fcfbf9', fontFamily: "'Inter', 'Noto Sans KR', sans-serif", color: '#2c2c2c' }}>
      <main className="flex-1 overflow-y-auto">
        {/* Top Bar */}
        <header className="h-16 bg-white border-b border-stone-200 flex items-center justify-between px-8 sticky top-0 z-10">
          <h2 className="text-xl font-bold" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>비용 및 예산 관리</h2>
