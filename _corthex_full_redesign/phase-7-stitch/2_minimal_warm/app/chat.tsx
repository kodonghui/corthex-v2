...
export function ChatPage() {
  ...
  return (
    <div data-testid="chat-page" className="h-screen overflow-hidden flex flex-col" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif", backgroundColor: '#fcfbf9', color: '#2c2c2c' }}>
      {/* BEGIN: Global Header */}
      <header className="h-16 border-b border-gray-200 bg-white/80 backdrop-blur-md flex items-center justify-between px-6 z-10 shrink-0">
        <div className="flex items-center gap-4">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold" style={{ backgroundColor: '#e57373' }}>C</div>
          <h1 className="text-xl font-bold tracking-tight" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>CORTHEX <span className="font-light" style={{ color: '#e57373' }}>v2</span></h1>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="w-10 h-10 rounded-full border border-gray-100 flex items-center justify-center overflow-hidden" style={{ backgroundColor: '#f5f3ec' }}>
            <span className="text-sm font-bold" style={{ color: '#e57373' }}>U</span>
          </div>
        </div>
      </header>
      {/* END: Global Header */}
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <aside className={`w-72 border-r border-gray-200 flex flex-col overflow-hidden ${showChat ? 'hidden md:flex' : 'flex'}`} style={{ backgroundColor: '#f5f3ec' }}>
          <div className="p-4">
            <button
            ...
            {sessions.map((session) => {
              ...
              return (
                <div
                  ...
                  style={isActive ? { borderLeft: '4px solid #e57373' } : {}}
                >
        ...
        <section className={`flex-1 flex flex-col relative ${!showChat ? 'hidden md:flex' : 'flex'}`} style={{ backgroundColor: '#fcfbf9' }}>
          <ChatArea
            agent={selectedAgent}
          ...
          <aside className="w-80 border-l border-gray-200 bg-white flex-col hidden lg:flex">
            <div className="p-8 text-center space-y-4">
              <div className="relative inline-block">
                <div className="w-24 h-24 rounded-3xl flex items-center justify-center mx-auto shadow-inner relative overflow-hidden" style={{ backgroundColor: '#f5f3ec' }}>
                  <svg className="h-12 w-12 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style={{ color: '#e57373' }}>
              ...
              <div>
                <h2 className="text-xl font-bold" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>{selectedAgent.name}</h2>
                <p className="text-xs font-bold tracking-widest uppercase mt-1" style={{ color: '#e57373' }}>{selectedAgent.role || 'Agent'}</p>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto px-6 space-y-8 pb-10">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(selectedAgent.allowedTools && selectedAgent.allowedTools.length > 0
                    ...
                    <span key={tool} className="px-3 py-1 text-[11px] font-semibold rounded-full" style={{ backgroundColor: '#f5f3ec', color: '#e57373' }}>
                      {tool}
                    </span>
                  ))}
                </div>
              ...
              {/* Context Summary */}
              <div className="p-4 rounded-2xl border border-gray-100" style={{ backgroundColor: '#f5f3ec' }}>
                <div className="flex items-center gap-2 mb-2">
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style={{ color: '#e57373' }}>
