    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          ...
          <textarea value={soul} onChange={(e) => setSoul(e.target.value)} className="w-full h-64 p-3 rounded-lg border border-slate-300 bg-white text-sm font-mono resize-y focus:outline-none focus:ring-2" style={{ focusRingColor: '#e57373' }} placeholder="에이전트의 Soul을 작성하세요..." />
          <div className="mt-2">
            <p className="text-[10px] font-medium text-slate-500 mb-1">사용 가능한 변수:</p>
            <div className="flex flex-wrap gap-1">
              {availableVars.map((v) => (
                <span key={v} className="px-1.5 py-0.5 rounded text-[10px] font-mono cursor-pointer" style={{ backgroundColor: 'rgba(229, 115, 115,0.1)', color: '#e57373' }} onClick={() => setSoul((prev) => prev + v)} title="클릭하여 삽입">{v}</span>
              ))}
            </div>
          </div>
        </div>
            ...
            <div className="mt-2 space-y-1">
              ...
              {Object.entries(preview.variables).map(([key, val]) => (
                <div key={key} className="text-[10px] font-mono text-slate-500 truncate">
                  <span style={{ color: '#e57373' }}>{`{{${key}}}`}</span>{' = '}<span>{val || '(빈 값)'}</span>
                </div>
              ))}
            </div>
          )}
    ...
    <div className="flex flex-col gap-6 p-6 bg-white rounded-xl border border-slate-200 shadow-sm">
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        <div className="flex items-center gap-6">
          <div className="w-24 h-24 rounded-2xl flex items-center justify-center shadow-inner relative" style={{ backgroundColor: '#f5f3ec' }}>
            <Bot className="w-10 h-10" style={{ color: '#e57373' }} />
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
              <h2 className="text-slate-800 text-3xl font-bold tracking-tight" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>{agent.name}</h2>
        ...
        <div className="flex gap-8">
          {detailTabItems.map((tab) => (
            <button
              ...
              style={detailTab === tab.value ? { borderColor: '#e57373', color: '#e57373' } : {}}
            >
              {tab.label}
            </button>
          ))}
      ...
      {detailTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-6">
              ...
              {recentActivities.map((act) => (
                <div key={act.id} className="p-4 flex items-center justify-between hover:bg-slate-100/50 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: act.icon === 'success' ? 'rgba(229, 115, 115,0.1)' : 'rgba(59,130,246,0.1)', color: act.icon === 'success' ? '#e57373' : '#3b82f6' }}>
          ...
          <div className="lg:col-span-1 flex flex-col gap-4">
            ...
            <div className="rounded-xl p-6 border border-slate-100 shadow-sm relative overflow-hidden" style={{ backgroundColor: '#f5f3ec' }}>
              <div className="flex flex-col gap-6 relative z-10">
                ...
                <div>
                  ...
                  <div className="w-full bg-slate-200 h-1.5 rounded-full mt-3 overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: '0%', backgroundColor: '#e57373' }} />
                  </div>
                </div>
                <div className="w-full h-px bg-slate-200" />
                <div>
...
export function AgentsPage() {
  ...
  if (isLoading) {
    return (
      <div data-testid="agents-page" className="flex-1 ml-64 flex flex-col min-h-screen" style={{ backgroundColor: '#fcfbf9' }}>
  ...
  return (
    <div data-testid="agents-page" className="flex min-h-screen" style={{ backgroundColor: '#fcfbf9', fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>
      {/* Sidebar */}
      <aside className="w-64 fixed inset-y-0 left-0 text-white flex flex-col z-50" style={{ backgroundColor: '#e57373' }}>
        <div className="p-6 flex items-center gap-3 border-b border-white/10">
          ...
          <div>
            ...
            <p className="text-xs text-white/70">Minimal Warm System</p>
          </div>
        </div>
      ...
      <main className="flex-1 ml-64 flex flex-col min-h-screen">
          ...
          <div className="flex items-center gap-4 flex-1 max-w-xl">
            <div className="relative w-full">
              ...
              <input
                ...
                style={{ backgroundColor: '#fcfbf9', focusRingColor: '#e57373' }}
        ...
        <div className="px-8 py-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            <div>
              <nav className="flex text-xs text-slate-400 mb-2 gap-2">
                <span>Workspace</span>
                <span>/</span>
                <span className="font-medium" style={{ color: '#e57373' }}>Agents</span>
              </nav>
              <h2 className="text-4xl font-bold text-slate-800" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>Agents Directory</h2>
              ...
              <div className="bg-white p-1 rounded-xl shadow-sm flex">
                {(['active', 'all', 'inactive'] as const).map((f) => (
                  <button
                    ...
                    style={filterActive === f ? { backgroundColor: '#e57373' } : {}}
                  >
          ...
          <div className="flex flex-wrap items-center gap-3 mb-8">
            ...
            <span className="px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1" style={{ backgroundColor: 'rgba(229, 115, 115,0.1)', color: '#e57373' }}>
          ...
          {agents.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {agents.map((agent, idx) => {
                ...
                return (
                  <div
                    ...
                    style={isSelected ? { borderColor: '#e57373' } : {}}
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: '#f5f3ec', color: '#e57373' }}>
                    ...
                    <h3 className="text-xl font-bold text-slate-800" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>{agent.name}</h3>
                    ...
                    <div className="mt-auto space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button className="hover:underline text-xs font-semibold flex items-center gap-1" style={{ color: '#e57373' }}>
              ...
              >
                <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 group-hover:text-white transition-all mb-3" style={{ ...(false ? { backgroundColor: '#e57373' } : {}) }}>
