...
export function ActivityLogPage() {
  ...
  return (
    <div className="swiss-grid" style={{ display: 'grid', gridTemplateColumns: '240px 1fr', minHeight: '100vh' }}>
      ...
      {/* Main Content */}
      <main className="flex flex-col h-screen overflow-hidden" style={{ backgroundColor: '#fcfbf9', color: '#463e30', fontFamily: "'Inter', sans-serif", gridColumn: '1 / -1' }}>
        {/* Header Section */}
        <header className="border-b px-8 py-6" style={{ backgroundColor: '#ffffff', borderColor: '#e8e4d9' }} data-purpose="page-header">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  style={{ backgroundColor: '#fcfbf9', borderColor: '#e8e4d9', color: '#463e30' }}
              ...
              <button className="p-2 rounded transition-colors" style={{ backgroundColor: '#2c2c2c', color: '#ffffff' }}>
          ...
          {/* Navigation Tabs */}
          <div className="flex border-b mt-8 gap-8" style={{ borderColor: '#e8e4d9' }} data-purpose="content-tabs">
            {TAB_ITEMS.map(item => (
              <button
                key={item.value}
        ...
        {/* Footer Pagination */}
        {totalCount > 0 && (
          <footer className="border-t px-8 py-3 flex items-center justify-between" style={{ backgroundColor: '#f2f0e9', borderColor: '#e8e4d9' }} data-purpose="list-pagination">
            ...
            <div className="flex gap-1">
              ...
              {Array.from({ length: Math.min(3, totalPages) }, (_, i) => i + 1).map(p => (
                <button
                  ...
                  style={page === p
                    ? { backgroundColor: '#2c2c2c', color: '#ffffff', fontWeight: 700 }
                    : { color: '#6a5d43' }
                  }
                >
                  {p}
    ...
    <>
      {Object.entries(grouped).map(([date, dateItems]) => (
        <div key={date} className="relative" style={{ paddingLeft: '10px' }}>
          {/* Timeline line */}
          <div className="absolute" style={{ left: '19px', top: 0, bottom: 0, width: '1px', backgroundColor: '#e8e4d9' }} />
          <h3 className="sticky top-0 py-2 text-xs font-bold uppercase tracking-widest mb-6 z-10" style={{ backgroundColor: '#fcfbf9', color: '#b7aa88' }}>
            {date}
          </h3>
  ...
  return (
    <div className="overflow-x-auto" data-testid="quality-table">
      <table className="w-full text-sm min-w-[560px]">
        <thead>
          <tr className="text-xs border-b" style={{ color: '#9c8d66', borderColor: '#e8e4d9' }}>
              ...
              <tr key={item.id} className="group">
                <td colSpan={5} className="p-0">
                  ...
                  >
                    ...
                    <div className="py-2.5 pr-3 text-xs truncate max-w-[200px] flex-1" style={{ color: '#2c2c2c' }}>{item.commandText || '-'}</div>
                    <div className="py-2.5 pr-3 min-w-[120px]">
                      {pct != null ? (
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ backgroundColor: '#e8e4d9' }}>
...
function QualityDetailPanel({ scores, feedback }: { scores: MergedScores; feedback: string | null }) {
  ...
  return (
    <div className="border-b" style={{ backgroundColor: 'rgba(242,240,233,0.5)', borderColor: '#e8e4d9' }} data-testid="qa-detail-panel">
      <div className="px-4 pt-2 flex gap-1 border-b" style={{ borderColor: '#e8e4d9' }}>
      ...
      {feedback && (
        <div className="px-4 pb-3">
          <p className="text-[11px] whitespace-pre-wrap border-t pt-2" style={{ color: '#9c8d66', borderColor: '#e8e4d9' }}>
            {feedback}
          </p>
        </div>
      )}
...
function DetailTabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      style={active
        ? { borderColor: '#2c2c2c', color: '#463e30', backgroundColor: '#f2f0e9' }
              ...
              return (
                <div key={rule.ruleId} className="flex items-start gap-2 px-2 py-1.5 rounded-lg" style={{ backgroundColor: 'rgba(242,240,233,0.5)' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-xs font-medium" style={{ color: '#2c2c2c' }}>{rule.ruleName}</span>
                    {rule.message && (
    ...
    <div className="space-y-2">
      {rubricScores.map((item) => (
        <div key={item.id} className="flex items-center gap-3 px-2 py-1.5 rounded-lg" style={{ backgroundColor: 'rgba(242,240,233,0.5)' }}>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium" style={{ color: '#2c2c2c' }}>{item.label}</span>
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex gap-4 text-xs">
          <span style={{ color: '#9c8d66' }}>총 주장: <span className="font-medium" style={{ color: '#2c2c2c' }}>{report.totalClaims}</span></span>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            {report.claims.length > 5 && (
              <button onClick={() => setShowAll(!showAll)} className="text-[10px]" style={{ color: '#2c2c2c', textDecoration: 'underline' }}>
          ...
          <div className="space-y-1">
            {(showAll ? report.claims : report.claims.filter(c => !c.verified || c.severity !== 'none').slice(0, 5)).map((cv, i) => (
              <div key={i} className="flex items-start gap-2 px-2 py-1 rounded-lg text-[10px]" style={{ backgroundColor: 'rgba(242,240,233,0.5)' }}>
                ...
                />
                <div className="flex-1 min-w-0">
                  <span className="font-medium" style={{ color: '#2c2c2c' }}>{cv.claim.value}</span>
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.id} className="transition-colors" style={{ borderBottom: '1px solid rgba(229,225,211,0.5)' }}>
              ...
              <td className="py-2.5 pr-3 text-xs font-medium font-mono" style={{ color: '#e57373' }}>{item.toolName}</td>
              <td className="py-2.5 pr-3 text-xs" style={{ color: '#2c2c2c' }}>{item.agentName || '-'}</td>
