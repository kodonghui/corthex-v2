    ...
    <div className="flex flex-col gap-6 bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
        ...
        <nav aria-label="Breadcrumb" className="flex text-sm text-slate-400">
          <ol className="inline-flex items-center space-x-1 md:space-x-2">
            <li className="inline-flex items-center">
              <button onClick={onClose} className="hover:underline transition-colors" style={{ color: '#e57373' }}>부서 목록</button>
            </li>
            <li>
      ...
      <div>
        <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
          <Users className="w-5 h-5" style={{ color: '#e57373' }} />
...
export function DepartmentsPage() {
  ...
  if (isLoading) {
    return (
      <div data-testid="departments-page" className="flex h-screen overflow-hidden" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>
        <div className="flex-1 p-8 space-y-6" style={{ backgroundColor: '#fcfbf9' }}>
  ...
  if (isError) {
    return (
      <div data-testid="departments-page" className="flex h-screen overflow-hidden" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>
        <div className="flex-1 p-8" style={{ backgroundColor: '#fcfbf9' }}>
  ...
  return (
    <div data-testid="departments-page" className="flex h-screen overflow-hidden" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif", backgroundColor: '#fcfbf9', color: '#0f172a' }}>
      {/* Sidebar */}
      <aside className="w-64 flex-shrink-0 border-r border-slate-200 bg-white hidden md:flex flex-col">
        <div className="p-6 flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white" style={{ backgroundColor: '#e57373' }}>
            <span className="material-symbols-outlined">eco</span>
          </div>
          <h2 className="text-xl font-bold tracking-tight" style={{ color: '#e57373' }}>CORTHEX v2</h2>
        </div>
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          <a className="flex items-center gap-3 px-3 py-2 rounded-xl shadow-sm text-white transition-colors" href="#" style={{ backgroundColor: '#e57373' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 flex-shrink-0 flex items-center justify-between px-8 bg-white/80 backdrop-blur-md border-b border-slate-200 z-10">
          ...
          <div className="flex items-center gap-4">
            <button className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg relative transition-colors">
              ...
              <span className="absolute top-2 right-2 w-2 h-2 rounded-full border-2 border-white" style={{ backgroundColor: '#e07a5f' }}></span>
            </button>
        ...
        <main className="flex-1 overflow-y-auto p-8">
            ...
            <div>
              <nav aria-label="Breadcrumb" className="flex mb-2">
                <ol className="flex items-center space-x-2 text-xs text-slate-400 font-medium">
                  ...
                  <li style={{ color: '#e57373' }}>부서 관리</li>
                </ol>
              </nav>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div className="border p-4 rounded-2xl flex items-center gap-4" style={{ backgroundColor: 'rgba(229, 115, 115,0.05)', borderColor: 'rgba(229, 115, 115,0.1)' }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'rgba(229, 115, 115,0.2)', color: '#e57373' }}>
                <span className="material-symbols-outlined">groups</span>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase" style={{ color: 'rgba(229, 115, 115,0.7)' }}>전체 부서</p>
            ...
            <div className="border p-4 rounded-2xl flex items-center gap-4" style={{ backgroundColor: 'rgba(236,91,19,0.05)', borderColor: 'rgba(236,91,19,0.1)' }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'rgba(236,91,19,0.2)', color: '#e07a5f' }}>
                <span className="material-symbols-outlined">smart_toy</span>
              </div>
              <div>
          ...
          {activeDepts.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-4 gap-6">
              {activeDepts.map((dept) => {
                ...
                const colors = ['rgba(229, 115, 115,0.1)', 'rgba(59,130,246,0.1)', 'rgba(236,91,19,0.1)', 'rgba(245,158,11,0.1)']
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4 overflow-hidden" style={{ backgroundColor: colors[colorIdx] }}>
                        <span className="material-symbols-outlined text-[28px]" style={{ color: '#e57373' }}>corporate_fare</span>
                      </div>
                      <button
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-bold" style={{ color: '#e57373' }}>--명</span>
                      </div>
                      <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: '50%', backgroundColor: '#e57373' }}></div>
                      </div>
          ...
          <div className="mt-12 bg-white rounded-2xl border border-slate-200 p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <a className="text-sm font-semibold hover:underline" href="#" style={{ color: '#e57373' }}>모든 내역 보기</a>
            </div>
            <div className="space-y-6">
              <div className="flex gap-4">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center" style={{ color: '#e57373' }}>
                  <span className="material-symbols-outlined">update</span>
                </div>
                <div className="flex-1 border-b border-slate-100 pb-4">
                  <p className="text-sm font-medium">부서 에이전트가 <span className="font-bold" style={{ color: '#e57373' }}>업데이트</span>되었습니다.</p>
                  <p className="text-xs text-slate-400 mt-1">방금 전 &bull; 시스템</p>
                </div>
              </div>
              <div className="flex gap-4">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center" style={{ color: '#e07a5f' }}>
                  <span className="material-symbols-outlined">person_add</span>
                </div>
                <div className="flex-1 border-b border-slate-100 pb-4">
                  <p className="text-sm font-medium">새로운 <span className="font-bold" style={{ color: '#e57373' }}>에이전트가 할당</span>되었습니다.</p>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium">부서 <span className="font-bold" style={{ color: '#e57373' }}>설정이 수정</span>되었습니다.</p>
