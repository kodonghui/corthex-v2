    ...
    >
      ...
      <div className="w-full max-w-7xl mx-auto px-6 sm:px-10 py-8 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between" data-testid="files-header">
          <h1 className="text-3xl font-bold leading-tight" style={{ fontFamily: "'Noto Serif KR', serif", color: '#463e30' }}>Files</h1>
          <div>
          ...
          {FILTER_OPTIONS.map(opt => (
            <button
              ...
              style={filter === opt.value
                ? { backgroundColor: '#5a7247', color: '#ffffff' }
                : { backgroundColor: '#f2f0e9', color: '#6a5d43', border: '1px solid #e5e1d3' }
              }
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-1">
            <button
              ...
              style={viewMode === 'list'
                ? { color: '#5a7247', backgroundColor: 'rgba(90,114,71,0.1)' }
                : { color: '#9c8d66' }
              }
              aria-label="리스트 보기"
            >
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" data-testid="files-loading">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-32 rounded-2xl animate-pulse" style={{ backgroundColor: '#f2f0e9', border: '1px solid #e5e1d3' }} />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          search || filter !== 'all' ? (
        ...
        {files.length > 0 && (
          <div className="flex items-center gap-4 px-1">
            ...
            <div className="flex-1 max-w-[200px] h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: '#e5e1d3' }}>
              <div className="h-full rounded-full" style={{ width: `${storagePercent}%`, backgroundColor: '#5a7247' }} />
            </div>
