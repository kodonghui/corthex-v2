...
export function DashboardPage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto w-full" style={{ fontFamily: "'Public Sans', sans-serif" }}>
      {/* Quick Stats Metrics */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <p className="text-sm font-medium text-slate-500">Total Companies</p>
            <span className="px-2 py-0.5 rounded text-[10px] font-bold" style={{ backgroundColor: 'rgba(90,114,71,0.1)', color: '#5a7247' }}>Active</span>
          </div>
          {isLoading ? (
            <div className="h-9 w-16 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" />
          ) : (
            <h3 className="text-3xl font-bold tracking-tight" style={{ fontFamily: "'Noto Serif KR', serif" }}>{depts.length}</h3>
          )}
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="px-2 py-0.5 rounded text-[10px] font-bold" style={{ backgroundColor: 'rgba(90,114,71,0.1)', color: '#5a7247' }}>+{users.filter(u => u.isActive).length}</span>
          </div>
          {isLoading ? (
            <div className="h-9 w-16 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" />
          ) : (
            <h3 className="text-3xl font-bold tracking-tight" style={{ fontFamily: "'Noto Serif KR', serif" }}>{users.length}</h3>
          )}
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="px-2 py-0.5 rounded text-[10px] font-bold" style={{ backgroundColor: 'rgba(90,114,71,0.1)', color: '#5a7247' }}>{agents.length}</span>
          </div>
          {isLoading ? (
            <div className="h-9 w-16 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" />
          ) : (
            <h3 className="text-3xl font-bold tracking-tight" style={{ fontFamily: "'Noto Serif KR', serif" }}>{agents.length}</h3>
          )}
          <p className="text-[10px] text-slate-400 mt-2">Total registered</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm" style={{ borderLeftWidth: '4px', borderLeftColor: '#5a7247' }}>
          <div className="flex justify-between items-start mb-4">
            <p className="text-sm font-medium text-slate-500">System Health</p>
            <span className="material-symbols-outlined" style={{ color: '#5a7247' }}>check_circle</span>
          </div>
          {isLoading ? (
            <div className="h-9 w-16 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" />
          ) : (
            <h3 className="text-3xl font-bold tracking-tight" style={{ fontFamily: "'Noto Serif KR', serif" }}>{onlineAgents}</h3>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h4 className="font-bold text-lg" style={{ fontFamily: "'Noto Serif KR', serif" }}>Revenue vs API Cost Trend</h4>
          ...
          <div className="h-[280px] w-full relative">
            <svg className="w-full h-full" viewBox="0 0 800 280">
              ...
              <path d="M0,200 Q100,180 200,120 T400,100 T600,60 T800,40" fill="none" stroke="#5a7247" strokeLinecap="round" strokeWidth="4" />
              {/* Cost Line Area */}
              ...
              <defs>
                <linearGradient id="revGradient" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="#5a7247" />
                  <stop offset="100%" stopColor="#5a7247" stopOpacity="0" />
                </linearGradient>
              </defs>
            </svg>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col">
          <h4 className="font-bold text-lg mb-1" style={{ fontFamily: "'Noto Serif KR', serif" }}>Active Sessions</h4>
          ...
          <div className="flex-1 flex items-end justify-between gap-1 h-full px-2">
            ...
            <div className="w-full rounded-t-sm" style={{ height: '95%', backgroundColor: '#5a7247' }} />
            <div className="w-full rounded-t-sm" style={{ height: '85%', backgroundColor: '#5a7247' }} />
            <div className="w-full rounded-t-sm" style={{ height: '100%', backgroundColor: '#5a7247' }} />
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-800 h-1 rounded-full overflow-hidden">
              <div className="h-full w-[85%]" style={{ backgroundColor: '#5a7247' }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h4 className="font-bold text-lg leading-none" style={{ fontFamily: "'Noto Serif KR', serif" }}>Recent System Alerts</h4>
            <p className="text-xs text-slate-400 mt-2">Latest security and operational logs</p>
          </div>
          <button className="text-sm font-bold hover:underline" style={{ color: '#5a7247' }}>View All Logs</button>
        </div>
                ...
                <>
                  {users.slice(0, 2).map((u) => (
                    <tr key={`user-${u.id}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="px-6 py-4">
                        <span className="flex items-center gap-1.5 font-bold text-xs uppercase" style={{ color: '#5a7247' }}>
                          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: '#5a7247' }} /> Info
                        </span>
                      </td>
                      <td className="px-6 py-4">
                  ...
                  {agents.slice(0, 2).map((a) => (
                    <tr key={`agent-${a.id}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="px-6 py-4">
                        <span className={`flex items-center gap-1.5 font-bold text-xs uppercase ${a.status === 'online' ? '' : 'text-amber-500'}`} style={a.status === 'online' ? { color: '#5a7247' } : {}}>
                          <span className={`w-2 h-2 rounded-full ${a.status === 'online' ? '' : 'bg-amber-500'}`} style={a.status === 'online' ? { backgroundColor: '#5a7247' } : {}} />
