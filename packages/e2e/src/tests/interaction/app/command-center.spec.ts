...
test.describe('Command Center — 인터랙션 테스트', () => {
  ...
  test('페이지 렌더링 및 핵심 요소 확인', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="command-submit"]')).toBeVisible()
  })

  test('빈 상태 표시 확인', async ({ page }) => {
    const emptyState = page.locator('[data-testid="message-empty-state"]')
  ...
  test('전송 버튼 활성화/비활성화', async ({ page }) => {
    const submitBtn = page.locator('[data-testid="command-submit"]')
    const input = page.locator('[data-testid="command-input"]')
  ...
  test('@멘션 입력 시 팝업 표시', async ({ page }) => {
    ...
    if (isVisible) {
      const items = page.locator('[data-testid="mention-agent-item"]')
      await expect(items.first()).toBeVisible()
    }
  })
