...
const inputClass = 'w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder:text-stone-500 focus:outline-none focus:ring-2 focus:border-transparent'
            ...
            {tabs.map(tab => (
              ...
              >
                <span className="text-sm font-semibold flex items-center gap-2" style={{ color: activeTab === tab.key ? oliveGreen : undefined }}>
                  {tab.icon} {tab.label}
                  {tab.count > 0 && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-200 text-stone-400 font-mono">{tab.count}</span>
                  )}
                </span>
              </button>
            ))}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr style={{ backgroundColor: `${oliveGreen}0d` }}>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-stone-400">에이전트명</th>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-stone-400">명령 내용</th>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-stone-400">상태</th>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-stone-400">실행 시간</th>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-stone-400 text-right">관리</th>
                </tr>
              </thead>
              <tbody className="divide-y" style={{ borderColor: `${oliveGreen}0d` }}>
                {isTabLoading && (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-sm text-stone-500" data-testid="jobs-loading">
                ...
                {!isTabLoading && activeTab === 'oneTime' && jobs.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-6 py-16 text-center" data-testid="jobs-empty">
                      <p className="text-sm text-stone-500">등록된 일회성 작업이 없습니다</p>
                    </td>
                  </tr>
                )}

                {!isTabLoading && activeTab === 'oneTime' && jobs.map(job => {
                  const status = STATUS_STYLES[job.status] || { label: job.status, dotClass: 'bg-slate-400', textClass: 'text-stone-400' }
                  return (
                    <tr
                      key={job.id}
                    ...
                    >
                      ...
                      <td className="px-6 py-5 text-sm text-slate-600">
                        ...
                        {expandedJob === job.id && job.result && (
                          <div className="mt-2 text-xs text-stone-400 bg-slate-50 rounded-lg p-3 max-h-40 overflow-y-auto whitespace-pre-wrap">{job.result}</div>
                        )}
                      ...
                      <td className="px-6 py-5 text-sm text-stone-400">
                      ...
                      <td className="px-6 py-5 text-right">
                        {job.status === 'queued' && (
                          <button onClick={(e) => { e.stopPropagation(); setDeleteTarget({ id: job.id, type: 'job' }) }} className="text-stone-500 hover:text-red-500 transition-colors"><Trash2 className="w-5 h-5" /></button>
                        )}
                        <button className="text-stone-500 transition-colors ml-1" onClick={(e) => e.stopPropagation()}><MoreVertical className="w-5 h-5" /></button>
                      </td>
                    </tr>
                  )
                })}
                ...
                {!isTabLoading && activeTab === 'schedule' && schedules.length === 0 && (
                  <tr><td colSpan={5} className="px-6 py-16 text-center" data-testid="schedules-empty"><p className="text-sm text-stone-500">등록된 반복 스케줄이 없습니다</p></td></tr>
                )}
                {!isTabLoading && activeTab === 'schedule' && schedules.map(s => (
                  <tr key={s.id} className="transition-colors" onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = `${oliveGreen}0d`)} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = 'transparent')} data-testid={`schedule-item-${s.id}`}>
                    ...
                    <td className="px-6 py-5">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${s.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-stone-400'}`}>
                        {s.isActive ? 'ACTIVE' : 'PAUSED'}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-sm text-stone-400">{s.description}{s.nextRunAt && ` (다음: ${new Date(s.nextRunAt).toLocaleString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })})`}</td>
                    <td className="px-6 py-5 text-right flex items-center justify-end gap-2">
                      <button onClick={() => openEditSchedule(s)} className="text-xs text-stone-500 hover:text-slate-600">편집</button>
                ...
                {!isTabLoading && activeTab === 'trigger' && triggers.length === 0 && (
                  <tr><td colSpan={5} className="px-6 py-16 text-center" data-testid="triggers-empty"><p className="text-sm text-stone-500">등록된 이벤트 트리거가 없습니다</p></td></tr>
                )}
                {!isTabLoading && activeTab === 'trigger' && triggers.map(t => (
                  <tr key={t.id} className="transition-colors" onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = `${oliveGreen}0d`)} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = 'transparent')} data-testid={`trigger-item-${t.id}`}>
                    ...
                    <td className="px-6 py-5">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${t.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-stone-400'}`}>
                    ...
                    <td className="px-6 py-5 text-right flex items-center justify-end gap-2">
                      <button onClick={() => openEditTrigger(t)} className="text-xs text-stone-500 hover:text-slate-600">편집</button>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => closeModal()}>
          <div className="bg-white border border-gray-200 rounded-2xl shadow-2xl max-w-lg w-full mx-4 p-6 space-y-4" onClick={e => e.stopPropagation()} data-testid="job-modal">
            ...
            <div>
              <label className="block text-xs font-medium text-stone-400 mb-1">담당 에이전트</label>
            ...
            <div>
              <label className="block text-xs font-medium text-stone-400 mb-1">작업 지시</label>
            ...
            {modalType === 'oneTime' && chainSteps.length === 0 && (
              <div>
                <label className="block text-xs font-medium text-stone-400 mb-1">실행 시간 (비워두면 즉시)</label>
            ...
            {modalType === 'schedule' && (
              <>
                <div>
                  <label className="block text-xs font-medium text-stone-400 mb-1">실행 시간</label>
            ...
            {modalType === 'trigger' && (
              <>
                <div>
                  <label className="block text-xs font-medium text-stone-400 mb-1">트리거 유형</label>
                ...
                {(modalTriggerType === 'price-above' || modalTriggerType === 'price-below') && (
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-stone-400 mb-1">종목 코드</label>
                      <input type="text" value={modalStockCode} onChange={e => setModalStockCode(e.target.value)} placeholder="005930" className={inputClass} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-stone-400 mb-1">목표가 (원)</label>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => closeModal()} className="flex-1 py-2.5 text-sm font-medium text-stone-400 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">취소</button>
              <button
