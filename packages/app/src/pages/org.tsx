...
const STATUS_CONFIG: Record<string, { dot: string; shadow?: string; pulse?: boolean; label: string; textColor: string }> = {
  ...
  offline: { dot: 'bg-slate-500', label: '오프라인', textColor: 'text-stone-400' },
}
...
const TIER_CONFIG: Record<string, { classes: string; label: string }> = {
  ...
  worker: { classes: 'bg-slate-500/15 text-stone-500', label: 'Worker' },
}
...
function AgentDetailPanel({ agent, onClose }: { agent: OrgAgent; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40" data-testid="agent-detail-panel">
      ...
      <div className="absolute right-0 top-0 h-full w-80 md:w-96 bg-stone-100 border-l border-stone-200 shadow-2xl overflow-y-auto">
        <div className="px-5 py-4 border-b border-stone-200 flex items-center justify-between">
          ...
          <button onClick={onClose} aria-label="닫기" className="text-stone-500 hover:text-slate-200 text-xl leading-none">✕</button>
        </div>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          {/* Info Fields */}
          <div>
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-wider">모델</p>
            <p className="text-sm text-slate-200 mt-0.5">{agent.modelName}</p>
          </div>

          <div>
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-wider">역할</p>
            <p className="text-sm text-slate-200 mt-0.5">{agent.role || '역할 미지정'}</p>
          </div>

          <div>
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-wider">계급</p>
          ...
          {/* Soul */}
          <div>
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-wider">SOUL</p>
            {soulText ? (
              <div className="bg-white/50 border border-stone-200 rounded-lg p-3 mt-1">
                <p className="text-xs text-stone-600 leading-relaxed max-h-40 overflow-y-auto whitespace-pre-wrap">{soulText}</p>
              </div>
            ) : (
              <p className="text-xs text-stone-400 italic mt-1">Soul이 설정되지 않았습니다</p>
            )}
          </div>

          {/* Tools */}
          <div>
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-wider">허용 도구</p>
            {tools.length > 0 ? (
              <div className="flex flex-wrap gap-1.5 mt-1">
                {tools.map((tool) => (
                  <span key={tool} className="text-[10px] bg-stone-200/50 text-stone-500 px-2 py-0.5 rounded-full">{tool}</span>
                ))}
              </div>
            ) : (
              <p className="text-xs text-stone-400 italic mt-1">도구가 할당되지 않았습니다</p>
            )}
          </div>
        </div>
      </div>
...
function AgentNode({ agent, isSelected, onSelect }: { agent: OrgAgent; isSelected: boolean; onSelect: (a: OrgAgent) => void }) {
  ...
  return (
    <div
      ...
      className={`bg-stone-100/80 border rounded-lg p-3 cursor-pointer hover:border-stone-300 hover:bg-stone-200/50 transition-all ${
        isSelected ? 'border-blue-500/30 bg-blue-500/5' : 'border-stone-200'
      }`}
    >
      ...
      <p className="text-[10px] text-stone-400 mt-1 truncate">{agent.role || '역할 미지정'}</p>
    </div>
  )
}
...
function DepartmentSection({ dept, selectedAgentId, onSelectAgent }: { dept: OrgDept; selectedAgentId: string | null; onSelectAgent: (a: OrgAgent) => void }) {
  ...
  return (
    <div data-testid={`dept-section-${dept.id}`} className="bg-stone-100/50 border border-stone-200 rounded-xl overflow-hidden">
      <div
        className="px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-stone-100/70 transition-colors"
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-stone-400 bg-stone-200/50 px-1.5 py-0.5 rounded">({dept.agents.length}명)</span>
        </div>
      </div>
      {expanded && (
        <div className="px-4 pb-4">
          ...
          {dept.agents.length === 0 && (
            <p className="text-xs text-stone-400 py-2">에이전트 없음</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="flex flex-col items-center py-6 px-4">
      ...
      <div className="relative w-full max-w-xs mx-auto mb-8">
        <div className="bg-stone-100/80 border-2 border-[#5a7247] rounded-xl p-4 shadow-lg relative z-10 flex flex-col items-center text-center">
        ...
        {org.departments.length > 0 && (
          <div className="absolute left-1/2 bottom-[-24px] w-px h-6 bg-stone-200 -translate-x-1/2" />
        )}
      </div>
          ...
          <div className={`grid gap-3 relative pt-6`} style={{ gridTemplateColumns: `repeat(${Math.min(org.departments.length, 3)}, minmax(0, 1fr))` }}>
            {org.departments.map((dept) => (
              <div key={dept.id} className="relative flex flex-col items-center">
                ...
                <div className="absolute top-[-24px] left-1/2 w-px h-6 bg-stone-200 -translate-x-1/2" />
                {/* Department node */}
                <div className="w-full bg-stone-100/80 border border-stone-200 rounded-lg p-3 flex flex-col items-center text-center cursor-pointer hover:border-stone-300 transition-colors">
                  <h3 className="text-sm font-semibold text-slate-50">{dept.name}</h3>
                  <div className="flex items-center mt-1.5 gap-1 text-xs text-stone-500">
                    <span>👥</span>
                    <span>{dept.agents.length}</span>
                  </div>
                </div>
                {/* Vertical connector to agents */}
                {dept.agents.length > 0 && <div className="w-px h-4 bg-stone-200" />}
                {/* Agent pills */}
                <div className="flex flex-col gap-1.5 w-full">
                  {sortAgentsByTier(dept.agents).slice(0, 3).map((agent) => {
                    ...
                    return (
                      <div
                        ...
                        className="bg-stone-100/80 border border-stone-200 rounded-lg p-2 flex items-center gap-2 cursor-pointer hover:bg-stone-200/50 transition-colors"
                      >
                        <div className="relative">
                          <div className="w-8 h-8 rounded-full bg-stone-200 flex items-center justify-center text-[10px] font-bold text-stone-600">
                            {agent.name.slice(0, 2)}
                          </div>
                          <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-stone-200 ${status.dot}`} />
                        </div>
                  ...
                  {dept.agents.length > 3 && (
                    <p className="text-[10px] text-stone-400 text-center">+{dept.agents.length - 3}명 더</p>
                  )}
                </div>
              </div>
            ))}
  ...
  if (isLoading || !data) {
    return (
      <div data-testid="org-page" className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-stone-100/30 border border-stone-200/50 rounded-xl animate-pulse h-40" />
        ))}
      </div>
    )
  }
    ...
    <div data-testid="org-page" className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">{org.departments.length}개 부서 · {totalAgents}명 에이전트</p>
        </div>
        {/* View mode toggle (mobile tree / list) */}
        <div className="flex items-center bg-stone-100 rounded-lg p-1 md:hidden">
          <button
            onClick={() => setViewMode('tree')}
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              viewMode === 'tree' ? 'bg-stone-200 text-slate-50 shadow-sm' : 'text-stone-500 hover:text-slate-200'
            }`}
          >
            트리
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              viewMode === 'list' ? 'bg-stone-200 text-slate-50 shadow-sm' : 'text-stone-500 hover:text-slate-200'
            }`}
          >
            리스트
          </button>
      ...
      {/* Empty state */}
      {isEmpty && (
        <div className="bg-stone-100/30 border border-dashed border-stone-200 rounded-xl p-12 text-center">
          <p className="text-4xl mb-3">🏢</p>
          <p className="text-sm font-medium text-stone-600">조직이 구성되지 않았습니다</p>
          <p className="text-xs text-stone-400 mt-1">관리자 패널에서 부서와 에이전트를 추가해주세요</p>
        </div>
      )}
      ...
      {org.unassignedAgents.length > 0 && (
        <div data-testid="unassigned-section" className="bg-amber-500/5 border border-amber-500/20 rounded-xl overflow-hidden">
          <div className="px-4 py-3 flex items-center gap-2">
            ...
            <span className="text-[10px] text-stone-400 bg-stone-200/50 px-1.5 py-0.5 rounded">({org.unassignedAgents.length}명)</span>
          </div>
          <div className="px-4 pb-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
