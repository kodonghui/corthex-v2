...
export function MentionPopup({ query, selectedIndex, agents, deptMap, onSelect, onClose }: Props) {
  ...
  return (
    <div
      ...
      role="listbox"
      aria-label="에이전트 멘션"
      className="w-64 bg-corthex-surface border border-corthex-border rounded-xl shadow-2xl overflow-hidden z-50 max-h-80 overflow-y-auto"
    >
      {/* Search */}
      <input
      ...
      {[...grouped.entries()].map(([deptName, agentsInDept]) => {
        ...
        return (
          <div key={deptName}>
            {/* Dept label */}
            <p className="px-3 py-1 text-xs font-medium text-corthex-text-secondary bg-corthex-surface sticky top-0">{deptName}</p>
            {/* Agent rows */}
            {agentsInDept.map((agent, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm hover:bg-corthex-elevated transition-colors ${
                    isSelected ? 'bg-corthex-elevated' : ''
                  }`}
                  onClick={() => onSelect(agent)}
                  onMouseDown={(e) => e.preventDefault()}
                >
                  {/* Avatar */}
                  <span className="w-7 h-7 rounded-full bg-corthex-elevated flex items-center justify-center text-xs font-bold text-corthex-text-secondary shrink-0">
