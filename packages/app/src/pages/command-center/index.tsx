...
export function CommandCenterPage() {
  ...
  return (
    ...
    >
      {/* Mobile tab bar */}
      <div className="flex md:hidden border-b border-corthex-border shrink-0">
        <button
      ...
      <main className="flex-1 flex w-full overflow-hidden">
        ...
        <div
          className={`flex-[8] flex flex-col border-r border-corthex-border relative overflow-hidden
            ${mobileTab === 'chat' ? 'flex' : 'hidden'} md:flex`}
        >
          {/* Thread Header */}
          <div className="sticky top-0 bg-corthex-bg/95 backdrop-blur-sm z-10 border-b border-corthex-border p-4 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-3">
              <svg className="w-5 h-5 text-corthex-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
        ...
        <div
          className={`flex-[4] bg-corthex-surface flex flex-col overflow-y-auto border-l border-corthex-border
            ${mobileTab === 'report' ? 'flex' : 'hidden'} md:flex`}
        >
          <div className="p-6 flex flex-col gap-6">
            {/* Session Info */}
            <div className="flex flex-col gap-3">
              <h4 className="text-xs font-semibold text-corthex-text-secondary uppercase tracking-wider">Session Info</h4>
              <div className="bg-corthex-elevated rounded-xl border border-corthex-border p-4 flex flex-col gap-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-corthex-text-secondary">Commands</span>
                  <span className="text-sm font-mono tabular-nums text-corthex-text-primary">{userMsgCount}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-corthex-text-secondary">Responses</span>
                  <span className="text-sm font-mono tabular-nums text-corthex-text-primary">{agentMsgCount}</span>
                </div>
                <div className="h-px w-full bg-corthex-border my-1" />
                <div className="flex justify-between items-center">
                  <span className="text-sm text-corthex-text-secondary">Active Agents</span>
            ...
            {/* Delegation Pipeline */}
            <div className="flex flex-col gap-3">
              <h4 className="text-xs font-semibold text-corthex-text-secondary uppercase tracking-wider">Delegation Pipeline</h4>
            ...
            {/* Deliverables */}
            <div className="flex flex-col gap-3">
              <h4 className="text-xs font-semibold text-corthex-text-secondary uppercase tracking-wider">Deliverables</h4>
              <DeliverableViewer
            ...
            {/* Presets */}
            <div className="flex flex-col gap-3">
              <h4 className="text-xs font-semibold text-corthex-text-secondary uppercase tracking-wider">Presets ({presets.length})</h4>
              <button
                data-testid="kpi-card-presets"
                onClick={() => setShowPresetManager(true)}
                className="bg-corthex-elevated rounded-xl border border-corthex-border p-3 text-sm text-corthex-accent hover:bg-corthex-elevated hover:border-corthex-border-strong transition-colors text-left"
              >
                프리셋 관리하기 →
              </button>
            </div>
