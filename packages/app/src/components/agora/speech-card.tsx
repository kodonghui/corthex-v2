...
const AGENT_THEMES = [
  { avatar: 'bg-corthex-accent/20 text-corthex-accent border-corthex-accent/30', bubble: 'bg-stone-100 border-stone-200', bubbleRadius: 'rounded-2xl rounded-tl-sm' },
      ...
      <div className={cn('flex flex-col gap-1', isRight ? 'items-end' : 'items-start')}>
        ...
        {isLong && (
          <button
            ...
            className="mt-1 text-xs text-corthex-accent hover:text-corthex-accent-hover transition-colors"
          >
            {expanded ? '접기' : '더 보기'}
          </button>
        )}
