        ...
        return (
            <div className="flex items-end gap-3 justify-end group">
                <div className="flex flex-col gap-1 items-end max-w-[80%]">
                    <p className="text-slate-400 text-xs font-medium px-2">{sender || 'User'}</p>
                    <div className="text-[15px] font-normal leading-relaxed rounded-2xl rounded-br-sm px-5 py-3.5 bg-[rgba(34,211,238,0.10)] text-cyan-400">
                        {content}
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="flex items-start gap-3 group">
            ...
            <div className="flex flex-col gap-1 items-start max-w-[80%] w-full">
                <p className="text-slate-400 text-xs font-medium px-2">{sender || 'Agent'}</p>

                {isProcessing && (
                    <div className="flex items-center gap-2 mb-1 px-2">
                        <Loader2 className="w-3.5 h-3.5 text-slate-400 animate-spin" />
                        <span className="text-xs font-mono text-slate-400">Searching the web...</span>
                    </div>
                )}
