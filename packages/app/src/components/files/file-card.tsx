...
export function FileCard({ filename, size, uploaderName, uploaderIcon, fileIcon, iconBgClass }: FileCardProps) {
    return (
        <div className="flex flex-col rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 hover:border-cyan-400/50 transition-colors group cursor-pointer relative overflow-hidden">
            <div className="flex items-start justify-between mb-4">
                <div className={`size-10 rounded flex items-center justify-center ${iconBgClass}`}>
                    {fileIcon}
                </div>
                <button className="text-slate-400 hover:text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity">
