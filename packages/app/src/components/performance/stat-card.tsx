...
export function StatCard({ title, value, icon, iconColorClass, trendValue, trendText, isPositive }: StatCardProps) {
    ...
    return (
        <div className="flex flex-col gap-2 rounded-xl p-6 border border-corthex-border bg-corthex-surface/40 shadow-sm">
            <div className="flex justify-between items-center">
                <p className="text-stone-400 text-sm font-medium leading-normal">{title}</p>
                <div className={iconColorClass}>{icon}</div>
            </div>
            <p className={`text-corthex-text-primary text-3xl font-mono font-bold leading-tight mt-2 ${iconColorClass === 'text-emerald-500' && value.includes('%') ? 'text-emerald-500' : ''}`}>{value}</p>
