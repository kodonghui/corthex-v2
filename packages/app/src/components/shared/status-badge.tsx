...
export function StatusBadge({ status }: StatusBadgeProps) {
    switch (status) {
        case 'active':
            return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[rgba(34,211,238,0.10)] text-cyan-400 border border-[rgba(34,211,238,0.20)] uppercase tracking-wider">ACTIVE</span>;
        case 'scheduled':
        ...
        case 'idle':
            return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-800 text-slate-400 border border-slate-700 uppercase tracking-wider">IDLE</span>;
        default:
            return null;
    }
}
