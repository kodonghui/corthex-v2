...
function statusBadge(status: string) {
  switch (status) {
    ...
    default:
      return <span className="px-2 py-0.5 rounded text-xs bg-zinc-500/20 text-zinc-400">{status}</span>
  }
}
...
export function ExecutionHistoryPanel({ workflowId }: Props) {
  ...
  return (
    <div className="border-t border-zinc-700 bg-zinc-900/50 max-h-64 overflow-auto">
      <div className="px-4 py-2 border-b border-zinc-800 flex items-center justify-between">
        <h4 className="text-xs font-semibold text-zinc-400">실행 기록</h4>
        <span className="text-xs text-zinc-500">{executions.length}건</span>
      </div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {executions.map((exec) => {
              ...
              return (
                <tr key={exec.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                  <td className="px-4 py-2">{statusBadge(exec.status)}</td>
                  <td className="px-4 py-2 text-zinc-400">{formatTime(exec.startedAt)}</td>
                  <td className="px-4 py-2 text-zinc-400">{formatTime(exec.completedAt)}</td>
                  <td className="px-4 py-2 text-zinc-400 truncate max-w-48">
                    {result?.message ?? '—'}
                  </td>
                </tr>
              )
