...
export function ShareToConversationModal({ reportId, reportTitle, onClose, onShared }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose} data-testid="share-conversation-modal">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-96 max-h-[70vh] shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between">
          <div>
            <h3 className="font-medium text-sm text-slate-50">메신저로 공유</h3>
            <p className="text-xs text-slate-400 mt-0.5 truncate">{reportTitle}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 min-h-0 max-h-80">
          {conversations.length === 0 && (
            <p className="text-xs text-slate-400 text-center py-8">대화방이 없습니다</p>
          )}
          {conversations.map((conv) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-slate-500">{conv.participantCount}명</p>
              </div>
