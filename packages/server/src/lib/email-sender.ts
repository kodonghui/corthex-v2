  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
      ...
      <div style="border: 1px solid #e4e4e7; border-top: none; border-radius: 0 0 8px 8px; padding: 20px;">
        <p style="font-size: 15px; font-weight: 600; margin: 0 0 8px;">${title}</p>
        ${body ? `<p style="font-size: 13px; color: #71717a; margin: 0 0 16px;">${body}</p>` : ''}
        ${actionUrl ? `<a href="${actionUrl}" style="display: inline-block; background: #4f46e5; color: white; padding: 8px 16px; border-radius: 6px; text-decoration: none; font-size: 13px; font-weight: 500;">확인하기</a>` : ''}
      </div>
