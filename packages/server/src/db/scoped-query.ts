...
export function getDB(companyId: string) {
  ...
  return {
    ...
    // WRITE — mark observations as reflected (Story 28.4, D28)
    markObservationsReflected: async (ids: string[]): Promise<void> => {
    ...
    // WRITE — insert agent memory from reflection (Story 28.4, D28)
    insertReflectionMemory: (data: {
      agentId: string
      content: string
      confidence: number
      category: string
      ...
      db.insert(agentMemories).values({
        companyId: companyId as any,
        agentId: data.agentId,
        memoryType: data.category === 'preference' ? 'preference' : data.category === 'knowledge' ? 'fact' : 'insight',
        key: `reflection_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        content: data.content,
        context: 'auto-generated by reflection cron',
        source: 'reflection',
        confidence: data.confidence,
      }).returning({ id: agentMemories.id }),

    // READ — get today's reflection cost for a company (Story 28.4, D28 cost control)
    getReflectionCostToday: async (): Promise<number> => {
      const [result] = await db.select({ total: sql<number>`COALESCE(SUM(cost_usd_micro), 0)::int` })
        .from(costRecords)
        .where(and(
          eq(costRecords.companyId, companyId as any),
          eq(costRecords.source, 'reflection'),
          gte(costRecords.createdAt, sql`DATE_TRUNC('day', NOW())`),
        ))
