...
### 4.3 인터랙션 개선
- 토론 생성: 주제 + 유형(일반/심층) + 참여자 선택 UX 개선. 참여자 선택은 기본적으로 팀장급(Manager) 에이전트가 모두 선택된 상태에서 시작하며, 사용자가 체크박스로 해제/추가 가능. 최소 2명 이상 필수
- Diff 뷰: 타임라인 상단에 "Diff 보기" 토글 버튼 배치 (**완료된 토론에서만 활성화**, 진행중 토론에서는 비활성). 활성화 시 사이드바이사이드 뷰: 좌측에 이전 라운드, 우측에 현재 라운드의 동일 참여자 의견을 나란히 비교 표시. 3라운드(심층) 토론에서는 Diff 토글 옆에 라운드 쌍 선택 드롭다운 표시 (Round 1→2 / Round 2→3, 기본값: 마지막 2라운드). 모바일/태블릿에서는 세로 스택이지만 참여자별로 묶어서 표시 (참여자 A: Round 1 → Round 2, 참여자 B: Round 1 → Round 2 순으로). 기존 timeline API 데이터를 클라이언트에서 재가공 (API 변경 없음)
- "사령관실로 돌아가기" → "채팅으로 돌아가기"로 텍스트 변경 (v1 군사 테마 잔재 제거)
- 진행중 토론 타임라인 auto-scroll 정책: 마지막 발언 카드가 뷰포트 내에 보이면 새 발언 추가 시 자동 스크롤. 사용자가 이전 발언을 읽느라 위로 스크롤하여 마지막 발언이 뷰포트 밖이면 자동 스크롤하지 않고, 하단에 "새 발언이 있습니다" 플로팅 버튼 표시 (클릭 시 최하단으로 이동)
...
| 6 | DebateResultCard | 채팅 내 토론 결과 카드 스타일 개선 (**관련 컴포넌트 — 채팅 페이지에서 사용되지만 agora 도메인 컴포넌트이므로 여기서 관리**) | components/agora/debate-result-card.tsx |
...
| isDiffMode | useState (boolean, default: false) | Diff 뷰 활성화 여부 |
| diffRoundPair | useState ([number, number]) | Diff 비교 대상 라운드 쌍 (예: [1, 2]) |
...
- [x] 합의/비합의 결과 뱃지 표시 (합의=초록, 비합의=빨강. '부분합의'는 v2 추가 상태로 일부 참여자만 합의한 경우 노랑 뱃지)
- [x] 책 형식 정리 — v1에 있었으나 v2에서는 Diff 뷰로 대체. 별도 '책 형식' 뷰는 구현하지 않음 (토론 결과는 타임라인 + Diff 뷰로 충분히 커버)
...
| `debate-new-statement-btn` | 새 발언 알림 버튼 | 진행중 토론에서 위로 스크롤 시 새 발언 안내 |
| `debate-diff-round-select` | Diff 라운드 쌍 선택 | 3라운드 토론에서 비교할 라운드 쌍 선택 |
...
| 14 | Diff 비활성 | 진행중 토론에서 Diff 토글 확인 | `debate-diff-toggle` 비활성(disabled) 상태 |
| 15 | 태블릿 레이아웃 | 768px 뷰포트 | 2컬럼 (목록+타임라인), `debate-participant-summary` 표시 |
| 16 | fromChat 복귀 | 채팅에서 토론으로 이동 후 | `debate-back-to-chat` 버튼 표시 |
