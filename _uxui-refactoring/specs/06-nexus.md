...
│ (탭에 따라 전환)      │
├─────────────────────┤
│ AI 명령 입력         │  ← 캔버스 뷰에서만 표시
│ 상태바               │
└─────────────────────┘
```

> **모바일 뷰 규칙**: AI 명령 입력란과 상태바는 **캔버스 뷰에서만** 표시. 채팅 뷰에서는 채팅 입력란만 표시.
...
### 4.4 접근성/키보드 인터랙션
- 키보드 단축키 표시: 헤더 또는 `?` 키로 단축키 오버레이 (Undo=Ctrl+Z, Redo=Ctrl+Y, Save=Ctrl+S, Delete=Del 등)
- Tab 키로 툴바 버튼 간 포커스 이동, Enter로 실행
- 포커스 가시성: 툴바 버튼에 `focus-visible:ring-2` 적용 (키보드 사용 시 보이도록)
...
| 13 | KeyboardShortcutOverlay | 단축키 안내 오버레이 (`?` 키) | pages/nexus.tsx (인라인) |
...
| **768px~1439px** (Tablet) | 캔버스 전체, 채팅은 오버레이/드로어 (아이콘 토글) |
...
- [x] 노드 간 연결 (엣지) 생성 (드래그 핸들)
- [x] 멀티선택 (Ctrl+클릭 또는 Shift+클릭)
- [x] 서브그래프 그룹핑 (compound parent)
- [x] Mermaid 가져오기/내보내기
- [x] 캔버스 저장/불러오기
...
| `nexus-keyboard-shortcuts` | 단축키 오버레이 | 단축키 안내 |
| `nexus-performance-warning` | 대용량 캔버스 경고 | 성능 경고 표시 |
...
| 15 | Undo/Redo (버튼) | 노드 추가 → Undo 버튼 클릭 | 노드 제거됨, Redo 버튼으로 복원 |
| 16 | Undo/Redo (키보드) | 노드 추가 → Ctrl+Z | 노드 제거됨, Ctrl+Y로 복원 |
| 17 | 단축키 오버레이 | `?` 키 입력 | `nexus-keyboard-shortcuts` 표시 |
