...
### 4.3 인터랙션 개선
- 종목 호버 시 빠른 정보 프리뷰 (데스크톱/태블릿: 마우스 호버 팝오버, 모바일: 종목 카드 탭 시 인라인 확장으로 등락률/미니차트 표시 — long-press는 드래그 정렬과 충돌하므로 사용 금지)
- 비교 모드 토글 버튼
...
- **자동 갱신 실패**: 60초 갱신 실패 시 종목 가격 옆에 "갱신 실패" 경고 아이콘 + 마지막 갱신 시각 표시. 3회 연속 실패 시 상단에 배너 "실시간 시세 연결이 불안정합니다" + 수동 갱신 버튼
...
- [x] 자동매매 상태 표시 — v1 6.3 기능. 자동매매 설정/실행은 별도 페이지(또는 모달)이지만, 포트폴리오 요약 영역에 자동매매 ON/OFF 상태 + 투자 성향(보수/균형/공격) 뱃지 + 최근 체결 내역 요약을 표시. CIO(분석)+VECTOR(실행) 분리 구조는 백엔드 로직이므로 UI에서는 실행 결과만 표시
...
2. Chart panel (center, flexible width) — stock price chart (candlestick or line), time period selector, portfolio summary (cash, holdings, total return). Clean, data-rich but not overwhelming.
