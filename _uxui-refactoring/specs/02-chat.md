...
- **빈 상태**: 에이전트 추천 카드로 첫 대화 유도 (ChatArea 빈 상태 + SessionPanel 빈 상태 모두)
...
| **~375px** (Mobile) | 1컬럼: 세션 목록 ↔ 채팅 전환 (showChat 토글) |

**모바일 특별 처리:**
- 뒤로가기 버튼으로 세션 목록 복귀
- 에이전트 모달: 풀스크린
- 세션 관리: 스와이프 제스처 또는 ... 메뉴

---

## 9. 기존 기능 참고사항

v1-feature-spec.md에 직접 대응하는 항목은 없지만, 에이전트와의 1:1 채팅은 v2에서 새로 추가된 핵심 기능.
...
- [x] 자동 세션 선택 (최근 세션)
- [x] 파일 첨부 (최대 5개, 이미지 미리보기, 다운로드 링크)
...
| `chat-file-preview` | 첨부 파일 미리보기 | 전송 전 파일 확인 |
...
| # | 테스트 | 동작 | 기대 결과 |
|---|--------|------|----------|
| 1 | 페이지 로드 | /chat 접속 | `chat-page` 존재, 로그인 안 튕김 |
| 2 | 세션 목록 표시 | 로드 완료 | `session-panel` + 세션 항목들 표시 |
| 3 | 세션 선택 | 세션 클릭 | `chat-area`에 메시지 표시, URL 업데이트 |
| 4 | 새 대화 | + 버튼 클릭 | `agent-modal` 표시 |
| 5 | 에이전트 선택 | 모달에서 에이전트 클릭 | 새 세션 생성, 채팅 시작 |
| 6 | 메시지 입력 | textarea에 텍스트 입력 | 전송 버튼 활성화 |
| 7 | 메시지 전송 | 텍스트 입력 후 전송 | 유저 메시지 말풍선 추가 |
| 8 | 빈 전송 방지 | 빈 텍스트로 전송 시도 | 전송 버튼 비활성화 |
| 9 | 모바일 뷰 전환 | 375px에서 세션 클릭 | 채팅 뷰로 전환, 뒤로가기 버튼 표시 |
| 10 | 모바일 뒤로가기 | 뒤로 버튼 클릭 | 세션 목록으로 복귀 |
| 11 | 반응형 | 375px 뷰포트 | 1컬럼 레이아웃 |
| 12 | 세션 메뉴 | ··· 버튼 클릭 | 이름 변경/삭제 메뉴 표시 |
| 13 | 세션 삭제 확인 | 삭제 클릭 | 확인 다이얼로그 표시 |
| 14 | 세션 이름 변경 | 이름 변경 클릭 후 입력 | 인라인 입력 표시, Enter로 확정 |
| 15 | 날짜 그룹 접기 | 그룹 헤더 클릭 | 해당 그룹 접힘/펼침 |
| 16 | 파일 첨부 | 📎 버튼 클릭 후 파일 선택 | 미리보기 칩 표시 |
| 17 | 에러 재시도 | 에러 발생 후 재시도 클릭 | 마지막 메시지 재전송 |
| 18 | 빈 상태 (세션 없음) | 세션 0개일 때 | `session-empty-state` 표시 |
| 19 | 스트리밍 중지 | 전송 후 중지 버튼 클릭 | 스트리밍 중단, 전송 버튼 복귀 |
| 20 | 도구 호출 카드 | 도구 사용 에이전트 메시지 확인 | `chat-tool-call-card` 표시 |
| 21 | 위임 패널 전환 | 위임 탭 클릭 (비서 에이전트) | `chat-delegation-panel` 표시 |
| 22 | 타이핑 인디케이터 | 메시지 전송 후 대기 | `chat-typing-indicator` 표시 |
| 23 | 연결 상태 배너 | WebSocket 끊김 시 | `chat-connection-banner` 표시 |
