...
- **에이전트 0명**: 에이전트 카드에 "0명" + "AI 에이전트를 추가해보세요" 안내
...
3. AI usage chart — stacked bar chart showing daily spend by provider (Anthropic=blue, OpenAI=green, Google=orange). Toggleable between 7-day and 30-day views. Hoverable bars with tooltip showing date and cost.
...
| `dashboard-empty` | 빈 상태 메시지 | 데이터 없을 때 |
...
| # | 테스트 | 동작 | 기대 결과 |
|---|--------|------|----------|
| 1 | 페이지 로드 | /dashboard 접속 | `dashboard-page` 존재 |
| 2 | 요약 카드 표시 | 로드 완료 | 4개 카드 모두 숫자 표시 |
| 3 | 사용량 차트 | 로드 완료 | 차트 바 표시 |
| 4 | 기간 토글 | 30일 토글 클릭 | 차트 데이터 변경 |
| 5 | 예산 바 | 로드 완료 | 프로그레스 바 + % 표시 |
| 6 | 만족도 차트 | 로드 완료 | 도넛 차트 + % 표시 |
| 7 | 만족도 기간 | 30d 클릭 | 데이터 변경 |
| 8 | 비용 카드 클릭 | 비용 카드 클릭 | /costs 페이지로 이동 |
| 9 | 퀵 액션 | 퀵 액션 버튼 클릭 | 명령 실행 or 페이지 이동 |
| 10 | 반응형 | 375px 뷰포트 | 1컬럼 레이아웃 |
| 11 | 로딩 스켈레톤 | 데이터 로딩 중 | 스켈레톤 표시 |
| 12 | 에러 상태 | API 실패 시뮬레이션 | `dashboard-error` 표시 |
| 13 | 예산 퍼센트 | 로드 완료 | `budget-percentage` 숫자 표시 |
| 14 | WS 상태 표시 | 페이지 로드 | `ws-status` 인디케이터 존재 |
