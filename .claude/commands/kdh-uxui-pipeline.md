...
### ⚠️ 이미지 vs 설명서 우선순위 (가장 중요한 규칙)

```
디자인 이미지와 설명서(spec)가 충돌할 때의 우선순위:

┌─────────────────────────────────────────────────────┐
│  디자인 이미지 = 레이아웃/구조/배치의 최종 기준       │
│  설명서(spec)  = 기능 요구사항/data-testid/상태 정의  │
└─────────────────────────────────────────────────────┘

구체적으로:
- 레이아웃 (2단→3단, 리스트→그리드 등) → 이미지가 정답
- 컴포넌트 배치/크기/비율 → 이미지가 정답
- 색상/톤/시각적 스타일 → 이미지가 정답
- 어떤 데이터를 표시하는지 → 설명서가 정답
- data-testid 목록 → 설명서가 정답
- 로딩/에러/빈 상태 정의 → 설명서가 정답
- API 호출/이벤트 핸들러 → 기존 코드 유지 (둘 다 아님)

설명서에 "현재 구조 유지" "소폭 개선"이라고 써 있어도,
이미지가 완전히 다른 레이아웃을 보여주면 → 이미지를 따른다.
이미지가 보여주는 것이 "목표 디자인"이다.
```
...
3. 디자인 이미지의 레이아웃/구조를 충실히 재현할 것 (이미지 = 최종 목표)
4. 설명서 11번의 data-testid 전부 추가
5. 기존 data-testid 삭제 금지
6. 새 파일 생성 최소화 (기존 파일 수정 선호)
7. 이미지와 설명서를 동시에 참고하되, 레이아웃은 이미지 우선
```
