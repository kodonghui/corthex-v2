...
function isAllowedDocPath(filePath) {
  ...
  // CORTHEX-specific allowed paths
  if (/(^|\/)_bmad-output\//.test(normalized)) {
    return true;
  }
  if (/(^|\/)_corthex_full_redesign\//.test(normalized)) {
    return true;
  }
  if (/(^|\/)context-snapshots\//.test(normalized)) {
    return true;
  }
  if (/(^|\/)party-logs\//.test(normalized)) {
    return true;
  }
  if (/(^|\/)review-report\//.test(normalized)) {
    return true;
  }
  if (/(^|\/)_bmad\//.test(normalized)) {
    return true;
  }
  if (/(^|\/)_references\//.test(normalized)) {
    return true;
  }
