...
```typescript
// tool-handlers/builtins/notebooklm-create-notebook.ts
...
```typescript
// tool-handlers/builtins/notebooklm-audio-briefing.ts
...
```typescript
// tool-handlers/builtins/notebooklm-mindmap.ts
export const notebooklmMindmap: ToolHandler = async (input, ctx) => {
  ...
  const format = String(input.format || 'mermaid')
    ...
    // 'json': 구조화된 JSON → 프론트엔드에서 자유롭게 렌더링
    // 'image': PNG 이미지
...
- NotebookLM이 생성한 마인드맵 → Mermaid 형식으로 변환 → NEXUS 캔버스에 자동 렌더링
- 이미 NEXUS에 Mermaid ↔ Cytoscape.js 양방향 변환이 있으므로, 바로 연결 가능
...
```typescript
// tool-handlers/builtins/notebooklm-slides.ts
...
```typescript
// tool-handlers/builtins/notebooklm-flashcards.ts
...
1. **새 에이전트 온보딩**:
   - 부서 지식 베이스 → 플래시카드 → 새 에이전트의 초기 메모리에 주입
   - "이 부서에서 가장 중요한 20가지를 요약한 카드"

2. **CEO 일일 브리핑 카드**:
   - 오늘의 주요 뉴스 + 시장 동향 → 5장 카드로 요약
   - 텔레그램으로 카드 이미지 전송

3. **직원 교육 자료**:
   - 새 정책/법률 변경 → 학습 카드 → 직원들에게 배포
...
```typescript
// tool-handlers/builtins/notebooklm-infographic.ts
...
```
현재 SNS 카드 뉴스 파이프라인:
  에이전트가 텍스트 생성 → 이미지 생성 → 5-10장 카드 → 게시

NotebookLM 강화 버전:
  에이전트가 주제 분석 → notebooklm_infographic(5장) → 텍스트 오버레이 → 게시
  → 더 전문적이고 데이터 기반인 카드 뉴스
```
...
```typescript
// 지식 폴더에 문서 추가 시 자동으로 NotebookLM 노트북 업데이트
// knowledge-injector.ts에 추가
...
```
에이전트 학습 내용 (agent_memories)
  ↓ 주기적 통합 (주 1회)
  ↓
notebooklm_create_notebook("에이전트 X의 학습 기록")
  ↓
부서 공유 노트북에 추가
  ↓
다른 에이전트들도 오디오 브리핑으로 학습 가능
```
...
```typescript
// cron-execution-engine.ts에 새 작업 유형 추가
...
### 8-2. 에이전트 Soul에 NotebookLM 인식 추가

```markdown
# 투자분석팀장 Soul (예시 추가 부분)

## 사용 가능한 특수 도구
- notebooklm_audio_briefing: 분석 보고서를 음성 브리핑으로 변환할 수 있습니다.
  CEO가 "음성으로", "듣고 싶다", "브리핑" 등을 요청하면 이 도구를 사용하세요.
- notebooklm_slides: 데이터를 프레젠테이션으로 변환합니다.
  "발표 자료", "슬라이드", "PPT" 요청 시 사용하세요.
- notebooklm_mindmap: 복잡한 분석을 시각적 마인드맵으로 만듭니다.
  NEXUS에서 확인할 수 있는 Mermaid 형식으로 생성하세요.
```
...
NotebookLM이 사용 불가할 때:

```typescript
// 오디오 폴백: Google TTS 또는 ElevenLabs
if (notebooklmFailed) {
  const text = await generateSummary(sources)  // LLM으로 요약
  const audio = await googleTTS(text, 'ko')     // Google TTS로 음성 변환
  return { audioUrl: audio.url }
}

// 마인드맵 폴백: LLM으로 직접 Mermaid 생성
if (notebooklmFailed) {
  const mermaid = await generateMermaid(sources) // Claude로 Mermaid 생성
  return { mindmapData: mermaid, format: 'mermaid' }
}

// 인포그래픽 폴백: 이미지 생성 AI
if (notebooklmFailed) {
  const image = await generateImage(prompt)  // DALL-E 또는 Gemini
...
CREATE TABLE notebooklm_notebooks (
  ...
  external_notebook_id VARCHAR(200),     -- Google 측 노트북 ID
  source_type VARCHAR(50) NOT NULL,       -- 'knowledge_folder', 'report', 'debate', 'manual'
  source_id UUID,                         -- knowledge_folders.id, reports.id 등
...
CREATE TABLE notebooklm_outputs (
  ...
  output_type VARCHAR(50) NOT NULL,       -- 'audio', 'mindmap', 'slides', 'flashcards', 'infographic'
  output_url TEXT,
  output_data JSONB,                      -- 마인드맵 데이터, 카드 내용 등
  duration_seconds INTEGER,               -- 오디오 길이
  generated_by UUID REFERENCES agents(id),
  delivered_via VARCHAR(50),              -- 'telegram', 'email', 'messenger', 'sns'
