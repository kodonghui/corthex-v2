...
### 2. 회사 생성
4. 사이드바 → 회사 → "Create" 클릭
5. 회사명: `풀플로우테크`, 슬러그: `fullflow-tech`
6. "생성" 클릭 → **확인**: 목록에 추가
7. 스크린샷: `screenshots/part4-01-company.png`

### 3. 온보딩 (해당 회사 선택 후)
8. 사이드바 → 온보딩 → 풀플로우테크 선택
9. Step 1 (회사 정보): 이미 입력됨, Next
10. Step 2 (부서 생성): `개발본부` 추가, `마케팅팀` 추가
11. Step 3 (에이전트 생성): `풀플로우비서` (역할: CEO 비서 및 일정 관리), `코드리뷰봇` (역할: 코드 리뷰 전문가)
12. Step 4 (직원 초대): CEO — 아이디 `ff-ceo`, 이름 `김풀플로우` → 초대
13. **확인**: 비밀번호 모달 표시 → 비밀번호 복사/기록
14. 추가 직원 — 아이디 `ff-dev01`, 이름 `박개발` → 초대
